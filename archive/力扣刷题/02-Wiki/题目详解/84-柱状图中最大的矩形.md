...
created: 2026-07-15
type: permanent
summary: "C++17 Hot 100 项目中的《84. 柱状图中最大的矩形 (Hard)》。"
---

# 84. 柱状图中最大的矩形 (Hard)

> **专题归类：** [[06-栈与堆|06-栈与堆]] · [[03-数组与矩阵|03-数组与矩阵]]
...
public:
    int largestRectangleArea(vector<int>& heights) {
        int ans = 0; for (int i = 0; i < heights.size(); i ++) { int h = heights[i]; for (int j = i; j < heights.size(); j ++) h = min(h, heights[j]), ans = max(ans, h * (j - i + 1)); } return ans;
    }
};
```
...
- [[739-每日温度|739-每日温度]] — 单调栈入门题，找右边第一个更大元素。84 是找左右两边第一个更小元素，可以看作 739 的对称问题。
- [[42-接雨水|42-接雨水]] — 同样是单调栈经典题，但计算逻辑不同：84 在弹栈时计算面积，42 在弹栈时计算可接水量。
