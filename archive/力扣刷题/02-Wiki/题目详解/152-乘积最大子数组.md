...
created: 2026-07-15
type: permanent
summary: "C++17 Hot 100 项目中的《152. 乘积最大子数组 (Medium)》。"
---
...
public:
    int maxProduct(vector<int>& nums) {
        int ans = nums[0], maximum = 1, minimum = 1;
        for (int x : nums) {
...
- [[53-最大子数组和|53-最大子数组和]] — Kadane 算法的加法版（无需维护最小值）
- [[300-最长递增子序列|300-最长递增子序列]] — 子数组 vs 子序列 DP 的区别
- [[238-除自身以外数组的乘积|238-除自身以外数组的乘积]] — 前缀积/后缀积的另一种应用
