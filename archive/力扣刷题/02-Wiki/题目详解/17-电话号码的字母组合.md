...
created: 2026-07-15
type: permanent
summary: "C++17 Hot 100 项目中的《17. 电话号码的字母组合 (Medium)》。"
---
    ...
    vector<string> letterCombinations(string digits) {
        ...
        for (char d : digits) {
            ...
            while (size --) {
                ...
                for (char c : key[d - '0']) q.push(cur + c);
            }
        }
        vector<string> ans;
...
- [[46-全排列|46-全排列]] — 理解"每层选择列表相同 vs 不同"的区别，巩固回溯通用模板。
- [[22-括号生成|22-括号生成]] — 同样是约束回溯，但括号生成多了左右括号数量的约束条件。
- [[39-组合总和|39-组合总和]] — 组合类回溯，每层选择列表与约束条件有所不同。
