...
created: 2026-07-15
type: permanent
summary: "C++17 Hot 100 项目中的《394. 字符串解码 (Medium)》。"
---
...
public:
    string decodeString(string s) {
        stack<int> nums; stack<string> strs; string cur; int num=0; for(char c:s){if(isdigit(c))num=num*10+c-'0';else if(c=='['){nums.push(num);strs.push(cur);num=0;cur.clear();}else if(c==']'){string t=cur;cur=strs.top();strs.pop();int n=nums.top();nums.pop();while(n --)cur+=t;}else cur+=c;}return cur;
    }
};
```
### 思路三：递归解法
...
public:
    string decodeString(string s) {
        ...
        for (char c : s) {
            ...
            else if (c == '[') st.push({std::move(current), number}), current.clear(), number = 0;
            else if (c == ']') {
                auto [prefix, repeat] = st.top(); st.pop();
                string part = current;
                current = std::move(prefix);
                while (repeat --) current += part;
            } else current += c;
        }
        return current;
    }
...
- [[20-有效的括号|20-有效的括号]] — 括号匹配是 394 的基础版。394 在括号匹配的基础上增加了数字控制和字符串拼接逻辑。
- [[739-每日温度|739-每日温度]] — 栈在顺序处理场景中的另一种经典应用（单调栈 vs 嵌套处理栈）。
