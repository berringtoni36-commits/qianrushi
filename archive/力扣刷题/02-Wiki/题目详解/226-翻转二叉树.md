...
created: 2026-07-15
type: permanent
summary: "C++17 Hot 100 项目中的《226. 翻转二叉树 (Easy)》。"
---
...
- **边界条件：** 空树直接返回 nullptr；单节点无需翻转。
- **核心约束：** 必须对每个节点都执行左右子树交换，叶子节点交换两个 nullptr 不影响。
...
public:
    TreeNode* invertTree(TreeNode* root) {
        if(!root) return nullptr; queue<TreeNode*> q; q.push(root); while(!q.empty()){auto p=q.front();q.pop(); swap(p->left,p->right); if(p->left)q.push(p->left);if(p->right)q.push(p->right);} return root;
    }
};
```
---
...
- [[101-对称二叉树|101-对称二叉树]] — 翻转是让左右不对称，对称是检查左右是否镜像，两者是逆向思维。
- [[104-二叉树的最大深度|104-二叉树的最大深度]] — 同为二叉树基础递归题，递归思维模式一致。
- [[114-二叉树展开为链表|114-二叉树展开为链表]] — 同样是原地修改树结构，使用了后序 + prev 指针技巧。
