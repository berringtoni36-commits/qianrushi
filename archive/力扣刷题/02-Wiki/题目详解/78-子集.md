...
created: 2026-07-15
type: permanent
summary: "C++17 Hot 100 项目中的《78. 子集 (Medium)》。"
---
...
public:
    vector<vector<int>> subsets(vector<int>& nums) {
        vector<vector<int>> ans; int total = 1 << nums.size(); for (int mask = 0; mask < total; mask ++) { vector<int> cur; for (int i = 0; i < nums.size(); i ++) if (mask >> i & 1) cur.push_back(nums[i]); ans.push_back(cur); } return ans;
    }
};
```
...
- [[46-全排列|46-全排列]] — 排列与子集的对比学习：排列关心顺序（visited），子集不关心顺序（start）；排列只在叶子记录，子集每个节点都记录。
- [[39-组合总和|39-组合总和]] — 组合问题的变体，加了目标和约束，且元素可重复选取（递归传 i 而非 i+1）。
