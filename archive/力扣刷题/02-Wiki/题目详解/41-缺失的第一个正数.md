...
created: 2026-07-15
type: permanent
summary: "C++17 Hot 100 项目中的《41. 缺失的第一个正数 (Hard)》。"
---
...
public:
    int firstMissingPositive(vector<int>& nums) {
        int n = nums.size();
        for (int& x : nums) if (x <= 0 || x > n) x = n + 1;
        for (int x : nums) if (abs(x) <= n) nums[abs(x) - 1] = -abs(nums[abs(x) - 1]);
...
**同类原地哈希问题：**
- [[287-寻找重复数|287-寻找重复数]] — 值在 [1, n] 范围内，数组长度 n+1，找重复的数。
...
- [[287-寻找重复数|287-寻找重复数]] — 同样是值范围 [1, n] 的原地哈希问题，但找的是重复的数而不是缺失的数。可以用原地哈希或链表判圈法。
