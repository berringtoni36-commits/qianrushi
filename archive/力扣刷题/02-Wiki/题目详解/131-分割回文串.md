...
created: 2026-07-15
type: permanent
summary: "C++17 Hot 100 项目中的《131. 分割回文串 (Medium)》。"
---
...
- [[46-全排列|46-全排列]] — 回溯核心思想一致：选择→递归→撤销，区别在于本题是"切割"而非"排列"。
- [[5-最长回文子串|5-最长回文子串]] — 回文判断的核心算法一致，DP 递推式完全相同。
- 93-复原IP地址 — 同样是字符串切割问题，但约束条件不同（需要正好 4 段，每段在 0-255 范围），切割类回溯的典型代表。
- [[139-单词拆分|139-单词拆分]] — 同样是字符串分割，但只需判断能否分割而非列出所有方案，可用 DP 而非回溯。
