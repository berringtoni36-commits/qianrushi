...
chapter: "第2章 文件IO"
order: 3
source: "https://subingwen.cn/linux/stat/"
author: "苏丙榅"
...
[[archive/大丙Linux教程/第2章 文件IO/index|← 返回本章目录]] · [[02 Linux系统IO|上一篇：Linux系统IO]] · [[04 文件描述符复制和重定向|下一篇：文件描述符复制和重定向 →]]
...
| `-i` | 显示文件的 MIME 类型 |
| `-F` | 设置输出字符串的分隔符 |
| `-L` | 跟随软连接，查看目标文件的类型/属性；不加 `-L` 时通常查看链接本身 |
...
>
> `file` 默认不跟随符号链接，通常报告链接本身；添加 `-L` 才跟随链接并检查目标文件。
...
- `Uid` ： 文件所有者名字和所有者ID
- `Gid` ：文件所有数组名字已经组ID
- `Access Time`：表示访问时间，但是否更新会受 relatime、noatime 等挂载选项和文件系统策略影响。
...
struct stat {
    ...
    mode_t       st_mode;            // 文件类型和权限；宽度由实现决定
...
文件类型信息存储在 `struct stat` 的 `st_mode` 成员中，类型为 `mode_t`；其宽度和位布局由实现决定，不能固定说成 16 位。可使用 `S_ISREG`、`S_ISDIR` 等宏判断类型。
