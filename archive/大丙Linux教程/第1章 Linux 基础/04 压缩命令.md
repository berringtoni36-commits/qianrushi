...
order: 4
source: "https://subingwen.cn/linux/commpress/"
author: "苏丙榅"
...
[[archive/大丙Linux教程/第1章 Linux 基础/index|← 返回本章目录]] · [[03 用户管理命令|上一篇：用户管理命令]] · [[05 查找命令|下一篇：查找命令 →]]
...
- `x`: 释放压缩文件内容
- `z`: 使用 `gzip` 处理 `.tar.gz`/`.tgz` 文件（解压时用于解压）
- `j`: 使用 `bzip2` 处理 `.tar.bz2` 文件（解压时用于解压）
- `v`: 解压缩过程中显示解压缩信息
- `f`: 指定压缩包的名字
