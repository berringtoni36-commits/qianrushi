...
order: 8
source: "https://subingwen.cn/linux/library/"
author: "苏丙榅"
...
[[archive/大丙Linux教程/第1章 Linux 基础/index|← 返回本章目录]] · [[07 GCC|上一篇：GCC]] · [[09 Makefile|下一篇：Makefile →]]

> 来源：[原文：静态库和动态库](https://subingwen.cn/linux/library/)

不管是 Linux 还是 Windows，库都是已经编译的目标代码集合；具体文件格式和后缀因平台而异。常见的库分为 `静态库` 和 `动态库`，它们不是源文件，而是由目标文件、符号和重定位等信息组成的二进制构件。
...
- 在 Windows 中静态库常见后缀为 `.lib`；前缀和命名约定取决于工具链，不应一概规定为 `libxxx.lib`。
...
```shell
# 2. 将生成的目标文件 .o 打包成静态库
$ ar rcs libcalc.a add.o div.o mult.o sub.o    # 也可以按需使用 *.o，但不要把 main.o 打进库
...
>
> 动态链接库是按特定格式组织的目标代码集合。代码通常使用位置无关代码或经过重定位，符号的最终地址由链接器和动态加载器结合装载地址、重定位及符号解析规则确定；不能简单说静态库使用绝对地址。
>
> 关于动态库的命名规则如下:

- 在Linux中动态库以 `lib` 作为前缀, 以`.so` 作为后缀, 中间是库的名字自己指定即可, 即: `libxxx.so`
- 在 Windows 中动态库通常使用 `.dll` 后缀；是否带 `lib` 前缀取决于工具链和项目约定，例如 `xxx.dll` 或 `libxxx.dll` 都可能出现。
...
- 静态库如何被加载
在最终链接阶段，链接器会从静态库中提取被引用的目标模块并把代码和数据合入可执行文件；运行时不再依赖该静态库文件本身，但仍可能依赖其他动态库。
- 动态库如何被加载
  - 链接阶段通常把动态库的名称及依赖关系写入可执行文件，而不是把整个动态库复制进去；`-L` 主要影响链接时搜索。
  - 程序启动时，动态加载器通常会映射直接依赖的共享对象；符号绑定可能按启动时或首次调用时进行，具体取决于链接和加载选项。
  - 通过 `dlopen` 显式加载的库则在运行期间按调用时机映射。

### 2.4.2 动态链接器

> 动态链接器是用户进程地址空间中的加载器组件，不是一个独立的应用进程。程序启动或调用 `dlopen` 时，它按可执行文件记录的依赖关系和搜索规则完成共享对象的映射、重定位和符号解析；链接时的 `-L` 不会自动成为运行时搜索路径。
>
> 那么动态链接器是如何搜索某一个动态库的呢，在它内部有一个默认的搜索顺序，按照优先级从高到低的顺序分别是：

搜索顺序不是所有系统都完全相同，通常会综合考虑 DT_RPATH/DT_RUNPATH、`LD_LIBRARY_PATH`、缓存和默认目录；安全执行、setuid 程序以及链接器版本还可能改变规则。应以目标系统的动态链接器文档和 `ld.so` 配置为准。
...
- 方案1: 将库路径添加到环境变量 LD\_LIBRARY\_PATH 中
  1. 找到相关的配置文件
    ...
    2. 使用 vim 打开配置文件, 在文件最后添加这样一句话
        ```shell
        # 自己把路径写进去就行了
        export LD_LIBRARY_PATH="/absolute/lib/dir${LD_LIBRARY_PATH:+:$LD_LIBRARY_PATH}"
        ```
    3. 让修改的配置文件生效
...
- 方案2: 更新 /etc/ld.so.cache 文件
  1. 找到动态库所在的绝对路径（不包括库的名字）比如： `/home/robin/Library/`
    2. 在 `/etc/ld.so.conf.d/` 下新增一个配置文件（例如 `myapp.conf`），每行写一个动态库目录；然后运行 `ldconfig`。直接改主配置文件前应先确认系统约定。
        ```shell
        # 1. 打开文件
...
- 优点：
  - 可实现不同进程间的资源共享
    ...
    - 直接依赖的动态库通常在程序启动时由动态加载器映射；需要显式控制运行时加载时可使用 `dlopen()`，不能简单归因于“是否调用了某个库函数”。
- 缺点：
