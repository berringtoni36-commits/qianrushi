...
order: 2
source: "https://subingwen.cn/linux/file-commands/"
author: "苏丙榅"
...
[[archive/大丙Linux教程/第1章 Linux 基础/index|← 返回本章目录]] · [[01 初识Linux操作系统|上一篇：初识Linux操作系统]] · [[03 用户管理命令|下一篇：用户管理命令 →]]
...
> 给 `ls` 添加 `-a` 参数（就是 `all ` 的意思）就可以显示指定目录中是所有文件了, 因为默认情况下具有隐藏属性的文件是不会显示出来的。那么在Linux中什么样是文件才能被隐藏呢？其实很简单只需要在文件名前边加一个点(`.`)文件就具有隐藏属性了， 例如： `.demo`, `.a.txt`
...
> 硬链接计数是一个整数，表示指向同一个 inode 的目录项数量。硬链接不会复制文件内容；多个名称通常共享同一 inode 和数据块，因此通过任一名称修改内容都能看到相同数据。创建新目录项和 inode 元数据仍可能占用少量目录空间；删除一个名称也不会在仍有其他硬链接或打开文件时立即释放数据。
>
...
- 文件大小 —> `单位是字节`
  - 如果文件是目录，显示的是目录项等元数据占用的大小，具体数值由文件系统和目录内容决定，不是固定的 `4096`，也不包括目录中普通文件的数据大小
...
- 拷贝目录 ==> `目录不存在得到新目录, 该目录被拷贝到存在的目录中`
    ...
    $ cp -r A/* B    # 拷贝 A目录中的非隐藏内容到目录B中；若包含子目录需要 -r
    ```
...
- 文件的移动
    ```shell
    # 语法: mv 要移动的文件  目标
    # 如果 B 已存在且是目录，A 会被移动到 B 中；如果 B 不存在，mv 可将 A 重命名为 B
    $ mv A B
    ```
- 文件改名
    ```shell
...
- cat
> 该命令会把文件内容写到标准输出；终端窗口只能显示有限的滚动历史，所以查看大文件时通常使用 `less`、`more`、`head` 或 `tail`，而不是把 `cat` 误认为只能读取小文件。
    ```shell
    $ cat 文件名
    ```
- more
