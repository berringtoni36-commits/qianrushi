...
order: 10
source: "https://subingwen.cn/linux/gdb/"
author: "苏丙榅"
...
[[archive/大丙Linux教程/第1章 Linux 基础/index|← 返回本章目录]] · [[09 Makefile|上一篇：Makefile]] · [[01 文件描述符|下一篇：文件描述符 →]]
...
如果通过 `s` 单步调试进入到函数内部，可以执行 `finish` 继续运行到当前函数返回。过程中若命中断点、信号或异常，GDB 仍可能先停下；优化或缺少调试信息也会影响定位。
...
`until` 可以继续运行到指定位置，或在当前循环之后停下；它受现有断点、信号、优化级别和调试信息影响，并不要求必须满足固定的“循环起始/结束行”条件。
```shell
(gdb) until
```
