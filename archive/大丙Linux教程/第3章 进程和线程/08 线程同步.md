...
chapter: "第3章 进程和线程"
order: 8
source: "https://subingwen.cn/linux/thread-sync/"
author: "苏丙榅"
...
[[archive/大丙Linux教程/第3章 进程和线程/index|← 返回本章目录]] · [[07 多线程|上一篇：多线程]] · [[09 线程池 - C语言版|下一篇：线程池 - C语言版 →]]
...
每个线程有自己的寄存器上下文，但 CPU 缓存通常按层级在核心之间部分私有、部分共享，并不是全部独占。编译器、CPU 缓存一致性和内存模型决定何时能观察到共享数据；正确性不能靠“稍后写回物理内存”来保证，而要使用同步原语。
...
互斥锁的内部布局、是否记录线程 ID 以及具体等待策略属于实现细节；使用接口保证同一时刻的互斥访问。工程上常按临界区和锁的粒度设计锁，锁的数量与共享资源、访问模式和性能目标有关，并非固定的一资源一锁规则。
...
`信号量` 是带计数的同步原语，可表示资源数量、事件或许可；它不等同于互斥锁。访问共享数据的临界区是否还需要互斥锁，取决于数据结构和同步设计，并不是所有信号量用法都必须再配一把 mutex。
...
通常应先获取代表资源可用性的信号量，再获取保护队列的 mutex；具体顺序取决于不变量，必须全局一致，否则可能死锁。，下面来分析一种死锁的场景：
