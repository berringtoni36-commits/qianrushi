...
chapter: "第3章 进程和线程"
order: 5
source: "https://subingwen.cn/linux/signal/"
author: "苏丙榅"
...
[[archive/大丙Linux教程/第3章 进程和线程/index|← 返回本章目录]] · [[04 共享内存|上一篇：共享内存]] · [[06 守护进程|下一篇：守护进程 →]]
...
- `sleep()` 本身只是休眠；如果期间收到信号，系统调用可能被中断并返回。信号由外部事件、定时器或其他系统机制产生，而不是由 sleep 自动产生。
...
| 9 | SIGKILL | 无条件终止进程；不能被捕获、阻塞或忽略 | 终止进程 |
| 10 | SIGUSR1 | 用户定义的信号。即程序员可以在程序中定义并使用该信号 | 终止进程 |
| 11 | SIGSEGV | 指示进程进行了无效内存访问(段错误) | 终止进程并产生core文件 |
| 12 | SIGUSR2 | 另外一个用户自定义信号，程序员可以在程序中定义并使用该信号 | 终止进程 |
| 13 | SIGPIPE | Broken pipe向一个没有读端的管道写数据 | 终止进程 |
| 14 | SIGALRM | 定时器超时，超时的时间 由系统调用alarm设置 | 终止进程 |
| 15 | SIGTERM | 可被捕获、阻塞或忽略；通常用于请求程序正常退出 | 终止进程 |
| 16 | SIGSTKFLT | Linux早期版本出现的信号，现仍保留向后兼容 | 终止进程 |
| 17 | SIGCHLD | 子进程结束时，父进程会收到这个信号 | 忽略这个信号 |
| 18 | SIGCONT | 如果进程已停止，则使其继续运行 | 继续/忽略 |
| 19 | SIGSTOP | 停止进程的执行；不能被捕获、阻塞或忽略 | 暂停进程 |
...
- 参数:
  - which: 定时器使用什么样的计时法则, 不同的计时法则发出的信号不同
    - `ITIMER_REAL`：按真实经过的墙上时间计时，发送 `SIGALRM`；不等同于进程实际获得的 CPU 时间。
        - `ITIMER_VIRTUAL`：只按进程在用户态消耗的 CPU 时间计时，发送 `SIGVTALRM`。
        - `ITIMER_PROF`：按进程在用户态和内核态消耗的 CPU 时间计时，发送 `SIGPROF`。
...
`sigset_t` 的具体布局、大小和信号数量由实现决定，应用程序不能假定它是 128 字节或 `int[32]`；应使用 `sigemptyset`、`sigaddset`、`sigismember` 等接口操作。

```shell
# 上图对信号集概念的示意
# 这里只是概念示意；信号编号、集合内部布局和位数由实现决定，不应据此推断固定的 1024 个标志位
            信号        标志位(从低地址位 到 高地址位)
               1      ->      0
              2             1
              3             2
...
#include <signal.h>
// 使用这个函数修改内核中的阻塞信号集
// sigset_t 的布局由实现决定，不要直接按 int[32] 或固定字节数访问
...
> 注意：信号处理函数中只能调用 async-signal-safe 操作。实际程序通常只设置 `volatile sig_atomic_t` 标志，在主循环中再执行 printf、文件操作或其他复杂逻辑。
...
int main()
{
    ...
    // SIGKILL/SIGSTOP 不能被捕获、阻塞或忽略，不能注册处理函数
