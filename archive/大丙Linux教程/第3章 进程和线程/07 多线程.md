...
chapter: "第3章 进程和线程"
order: 7
source: "https://subingwen.cn/linux/thread/"
author: "苏丙榅"
...
[[archive/大丙Linux教程/第3章 进程和线程/index|← 返回本章目录]] · [[06 守护进程|上一篇：守护进程]] · [[08 线程同步|下一篇：线程同步 →]]
...
- 进程有自己独立的地址空间, 多个线程共用同一个地址空间
  - 线程更加节省系统资源, 效率不仅可以保持的, 而且能够更高
    - 每个线程有自己的栈、寄存器上下文和线程局部存储；缓存是否共享取决于缓存层级和处理器架构。
    - 线程通常共享代码段、堆、全局数据和进程的描述符表，但访问共享数据仍需同步。
- 线程是调度执行的基本实体之一，进程是资源隔离和分配的重要边界；具体调度对象和实现由内核决定。
  - 一个进程可以包含多个可运行线程，多核系统可同时运行多个线程；线程数增加不等于获得线性更多 CPU 时间或更高性能。
...
1. I/O 密集任务可以用并发隐藏等待，但线程数应结合阻塞时间、内存、连接数和调度开销压测确定，不存在固定的 2 × 核心数最优值。
2. CPU 密集任务通常从不超过可用逻辑 CPU 数量的线程数起步，再根据亲和性、同步和负载测试调整。

## 2\. 创建线程

## 2.1 线程函数

每一个线程都有线程 ID，类型为 `pthread_t`；其具体表示由实现决定，不能假定一定是无符号长整形。要获取当前线程 ID 可以调用：

> 注意：下方示例用 `%ld` 打印线程 ID 只适用于特定实现，不能作为可移植写法；可移植程序应避免直接格式化 `pthread_t`，或为目标平台提供专门的显示辅助函数。
...
线程退出后，其可回收资源需要通过 `pthread_join()` 或分离状态回收；任意具有权限的线程都可以等待目标线程，不限于主线程。`pthread_join()` 通常会阻塞到目标线程退出，一次等待一个目标线程。
...
线程库提供了 `pthread_detach()`；线程进入分离状态后，退出时其可回收资源由线程实现自动回收，其他线程不能再对它 `pthread_join()`。这不等同于把资源交给其他进程。
...
pthread_t 的具体表示由实现决定；某些 Linux 实现可以把它表示为整数，但应用程序不应依赖这一点，也不能直接用 == 比较所有平台上的线程 ID。应使用 pthread_equal() 比较两个线程 ID，函数原型如下：
