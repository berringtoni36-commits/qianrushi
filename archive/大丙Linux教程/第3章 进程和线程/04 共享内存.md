...
chapter: "第3章 进程和线程"
order: 4
source: "https://subingwen.cn/linux/shm/"
author: "苏丙榅"
...
[[archive/大丙Linux教程/第3章 进程和线程/index|← 返回本章目录]] · [[03 内存映射区|上一篇：内存映射区]] · [[05 信号|下一篇：信号 →]]
...
```
- 参数:
  - key：用于标识 System V 共享内存对象；`IPC_PRIVATE`（通常为 0）也是有效的创建方式，具体 key 由调用者和系统约定。
...
shmget() 的 key 不要求一概为正数；`ftok` 可能失败并返回 -1，生成的 key 也应按实际返回值处理。该函数的函数原型如下：
...
`共享内存` 和 `内存映射区` 都可以实现进程间通信，但机制和生命周期不同：

- 实现方式：System V 共享内存对象不属于某个进程，进程通过 `shmat` 关联；文件映射则把各进程的虚拟地址区映射到同一文件或共享对象。
- 性能：两者都可能避免大量用户态拷贝，实际性能取决于访问模式、缺页、同步和文件系统；不能简单断言 mmap 一定更慢或一定更适合实时场景。
- 生命周期：映射通常随进程解除映射或退出而消失；System V 共享内存对象可在进程退出后继续存在，直到显式删除或系统清理。
- 持久性：文件映射可以通过适当的同步操作把数据写回文件，但并不保证每次写入立即持久化；共享内存本身是易失的，断电不会保留数据。
