---
title: "IO多路转接（复用）之epoll"
chapter: "第4章 套接字通信"
order: 9
source: "https://subingwen.cn/linux/epoll/"
author: "苏丙榅"
...
[[archive/大丙Linux教程/第4章 套接字通信/index|← 返回本章目录]] · [[08 IO多路转接（复用）之poll|上一篇：IO多路转接（复用）之poll]] · [[10 基于UDP的套接字通信|下一篇：基于UDP的套接字通信 →]]
...
- select 和 poll 每次调用都需要在用户态与内核态之间传递/检查集合；epoll 由内核维护监听对象和就绪事件，用户态通过 epoll_wait() 取回就绪事件，不能简单说成基于 mmap 共享内存或完全没有拷贝。
...
实际使用 epoll 的边沿触发（ET）模式时，应配合非阻塞 socket，并循环读取直到 read()/recv() 返回 -1 且 errno 为 `EAGAIN` 或 `EWOULDBLOCK`；这表示当前没有更多数据可读，不是异常。若直接使用阻塞 socket，循环读取可能在数据耗尽后阻塞。
