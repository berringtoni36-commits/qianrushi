---
title: "IO多路转接（复用）之select"
chapter: "第4章 套接字通信"
order: 7
source: "https://subingwen.cn/linux/select/"
author: "苏丙榅"
...
[[archive/大丙Linux教程/第4章 套接字通信/index|← 返回本章目录]] · [[06 套接字通信类的封装|上一篇：套接字通信类的封装]] · [[08 IO多路转接（复用）之poll|下一篇：IO多路转接（复用）之poll →]]
...
在 `select()` 函数中第2、3、4个参数都是 `fd_set` 类型，它表示一个文件描述符的集合，类似于信号集 `sigset_t` ，在 Linux 的常见实现中 FD_SETSIZE 通常为 1024、底层位图常见为 128 字节，但这些都是实现/头文件约定；它只是 select() 的可监视描述符上限，不等于内核文件描述符表的容量。

```c
# Linux 常见实现示例：FD_SETSIZE 可能为 1024；不要将该大小当作可移植保证
```

fd_set 的每一位通常按文件描述符的数值作为索引，用于表示该描述符是否加入集合；这不等于内核文件描述符表的容量，也受 FD_SETSIZE 等实现限制。
...
3. `select 能检测的描述符编号受 FD_SETSIZE 等用户态接口/头文件约定限制；Linux 常见值为 1024，但这不是内核文件描述符表容量，也不是所有平台的固定值。`
