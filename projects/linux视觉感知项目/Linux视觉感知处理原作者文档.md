---
title: 基于 FT2000/4 的嵌入式视觉感知系统解析
aliases:
  - Linux视觉感知处理原作者文档
tags:
  - 项目复盘
...
> [!abstract] 项目概览
> - **平台**：飞腾 FT2000/4、ARM Linux、银河麒麟 V10
> - **处理链路**：摄像头采集 → LIME 低照度增强 → 车道线模型推理 → QT 上位机展示
> - **核心优化**：循环重排、循环展开、ARM NEON、OpenMP、模型轻量化与端侧部署
> - **对比模型**：U-Net 语义分割、Transformer-based LSTR 端到端检测

# 基于 FT2000/4 的嵌入式视觉感知系统解析
...
**第一部分：低照度增强预处理算法的开发和优化：**对一篇学术论文《LIME》进行了代码实现，作为车道线识别之前的低照度增强预处理算法。然后由于开发板没有GPU加速，只能用CPU进行图像处理，并且对实时性要求比较高。所以对我复现的算法进行了程序性能优化：数据存储策略优化如循环重排（行优先遍历，外用列，确保数据存储顺序与遍历顺序一致，避免不必要的内存跳跃）提升缓存命中率和利用率，编译器指令集优化如 NEON 指令集（同时处理四个32位的浮点数）实现并行计算、循环展开（利用neon，减少循环控制的开销，同时提高指令级并行性），OpenMP多线程接口(多核处理块)等优化策略，显著提高预处理速度。

**第二部分：车道线识别神经网络的训练和部署：**模型部署方面，对比两种模型，分别是Unet和基于transformer的LSTR实现对车道线识别神经网络进行识别，采用深度可分离卷积替换传统卷积、结合参数量化技术来减少参数量；基于NCNN推理框架完成轻量化模型的端侧部署，实现模型的高效推理。

**第三部分：上位机与性能检测：**上位机展示部分：将神经网络推理后的画面显示在QT上位机中。还做了一个性能检测模块，采集CPU和内存的使用率。使用QT上的timer定时器 并利用终端上得top等工具采集系统CPU和内存 占用率显示在上位机界面中。
...
本项目在3.3.5中阐述了面向多核平台的OpenMP并行多线程加速接口，结合OpenMP多线程并行加速的特性，对LIME算法中图像增强函数进行色彩通道分离，每个色彩通道的读取、计算与存储都是有单独的一个CPU线程执行的，通过多核线程并行的加速方式，可以更加快速地处理色彩通道当中的复杂矩阵运算，从而加速代码执行速度；另一方面，利用OpenMP的多线程并行特性，执行图像分块操作，将图像分成四等份，每一份使用OpenMP多线程并行指令调用CPU核进行并行处理。
...
> [!warning] 代码阅读说明
> 以下代码根据原文重新排版并按功能分段；原文中少量变量、括号和边界处理存在不完整之处，面试复盘时应结合实际工程源码核对，不建议直接复制编译。

##### 3.3.4.1 用 NEON 优化求取最大值函数
...
| 指标 | 原始 U-Net | 深度可分离卷积 U-Net |
| --- | --- | --- |
|DICE|0.930|0.845|
|python下在x86平台的推理速度|1.59 seconds|1.06 seconds|
|权重文件大小|124MB|24MB|
> [!note] 表 6 补充说明
> 在 CPU 全力运转环境下，参数量减少带来的加速比将会更加明显，甚至速度能够提升至 2.5～3 倍；权重文件大小是在 float32 精度下生成的。

其中精度掉了大约0.085，但是速度提升了接近1.5倍。虽然通道可分离卷积在实践中具有许多优点，但也存在一定的精度损失。

#### 4.2.4 部署ncnn框架

##### 4.2.4.1 ncnn 嵌入式部署框架
...
以下示例按执行流程拆分展示；合并四段代码后，仍属于同一个 main() 示例。
...
ncnn::Mat in(image.rows*image.cols*3, data);
    ...
    ex.input("in0", in);
    ncnn::Mat mask;
    ex.extract("out0", mask); std::cout << "whc " << mask.w << " " << mask.h << " " << mask.c << std::endl;
```
...
**为什么用Transformer检测车道线？**

传统CNN（卷积）有一个天生的局限：每次只能看一小块局部区域（卷积核就那么大，如3×3），要看到更大范围就得堆很多层慢慢扩大视野。而车道线是一条跨越大半张图的连续长线，还经常被前车遮挡、在雨天夜间某段看不清。CNN只看局部，遇到断开的地方就容易"以为车道线到此为止"。

Transformer的注意力机制天生就能"一眼看全图"——图上任意两个位置之间都能直接建立联系。所以它能推理出"虽然中间被车挡住了，但根据上下两段的走向，它们其实是同一条车道线"。这就是"捕捉车道线长距离连续性"的含义，也是LSTR选择Transformer的根本原因。

**端到端四段式架构**

LSTR摒弃传统CNN依赖局部卷积的局限，采用"**特征提取骨干网络（Backbone）→ Transformer编码器（Encoder）→ Transformer解码器（Decoder）→ 车道线预测头（含FFNs）**"的端到端架构。可以把它想象成一个"看照片画车道线"的团队，四个成员依次接力：

| 成员 | 通俗职责 | 一句话作用 |
| --- | --- | --- |
| Backbone | 把照片压缩成要点 | 从原始图像提取低分辨率、高语义的特征 |
| Encoder | 通读全图、理解上下文 | 捕捉特征的全局关联，解决车道线连续性问题 |
| Decoder | 分配名额，每条车道派一个专员 | 结合初始查询与编码特征，生成车道级特征 |
| 预测头（FFNs） | 把专员的理解翻译成曲线 | 输出车道线曲线参数，完成检测 |

所谓"端到端"，是指图片进去、直接吐出车道线曲线参数，中间不需要人工做"分割→找点→聚类→拟合"的多步后处理。这既减少了出错环节，也是它能在开发板上跑得快的关键。

#### 4.3.2 结构具体介绍及其输入输出分析

##### 4.3.2.1 特征提取骨干网络（Backbone）：图像 → 低维语义特征

**一句话理解：** 把一张又大又啰嗦的原始照片，压缩成一张又小又精炼的特征图。就像把一篇5000字的文章做成500字的重点笔记——信息量小了，但关键内容都还在。

Backbone采用预训练的ResNet-18/34，移除全连接层后只保留卷积层、池化层与BatchNorm层，核心作用是将高分辨率原始图像转换为低分辨率、高语义的特征图。

**为什么要移除全连接层？** ResNet本来是个分类器，结构是"卷积层们（提取特征）→ 全连接层（做'这是猫/狗/车'的分类结论）"。卷积/池化层负责看图、提取特征，是我们要的；全连接层负责下分类结论，是我们不要的。因为LSTR只需要提取好的特征图交给Transformer，不需要分类结论，所以砍掉最后负责下结论的全连接层。"预训练"则意味着这个网络已在海量图片上学会了识别边缘、纹理、轮廓等通用特征，拿来即用，不必从零教它"什么是线条"。BN层保留下来是作为"稳定器"，让数据分布稳定、网络训练更稳。

**输入输出对比**

| 维度 | 输入（原图） | 输出（特征图） | 变化说明 |
| --- | --- | --- | --- |
| 批量 B | B | B（不变） | 一次处理几张图，不受影响 |
| 通道 | **3**（红/绿/蓝） | **256**（C） | 从颜色信息变成256种抽象语义特征 |
| 高 H | **368** | **23** | 缩小约16倍（池化层下采样） |
| 宽 W | **640** | **40** | 缩小约16倍（池化层下采样） |

**盯住维度变化就能读懂这一节：** 输入 [B,3,368,640] → 输出 [B,256,23,40]。

- **高宽变小（368→23、640→40，约缩小16倍）：** 这就是"下采样8~16倍"，由池化层完成，目的是丢掉冗余像素、减小计算量，对应原文的"低分辨率"。
- **通道从3变成256：** 原图的3通道只是最朴素的红绿蓝颜色信息；输出的256通道是256种抽象语义特征（可以想象成第1层标记"哪里像竖直线条"、第2层标记"哪里像路面"、第3层标记"哪里像护栏边缘"……）。尺寸虽小了（低分辨率），但每个位置蕴含的含义变浓了（高语义），这是一个"用空间分辨率换语义深度"的过程。

注意此时CNN只是局部地看到这些纹理和轮廓，要把它们全局串联起来（如"被挡住的两段其实是一条线"），是下一站Encoder的工作。

##### 4.3.2.2 Transformer 编码器（Encoder）：特征图 → 全局关联序列

**一句话理解：** 把920个"只知道自己局部信息的点"，变成920个"了解全图情况的点"——维度不变，但每个点变聪明了。

Encoder的核心组件是多层"多头自注意力（MHSA）+ 前馈网络（FFN）"，整体分三步完成"局部→全局"的转变：

**第一步：展平成序列。** Transformer不认识二维图片，只接受一维序列（像处理句子一样一个词接一个词）。所以先把 [B,256,23,40] 的特征图按空间位置摊平成 [B,920,256]（23×40=920），每个元素代表特征图上一个空间位置的256维特征，这就是"压缩序列S"。

**第二步：加回位置信息。** 把二维棋盘上的920颗棋子逐个捡起排成一排后，就丢失了"每颗棋子原来在哪个格子"的信息。而自注意力要计算任意两个位置的关系，必须知道它们在图里是相邻还是相隔很远。所以用正弦/余弦函数为每个位置生成一个独特的"位置指纹"（位置嵌入Eₚ，维度与S一致），再和序列直接相加：Encoder实际输入 = S + Eₚ。

**第三步：多头自注意力MHSA。** 这是核心——序列里每个位置都去问其余919个位置"你跟我有多相关？"，算出相关性分数后，用这些分数加权融合所有其他位置的信息来更新自己。于是一段被遮挡的车道线，即使自己看不清，也会因为和同一条线的其他片段相关性高，把它们的信息融进来，从而"知道"自己是完整车道线的一部分。这正是Encoder能捕捉长距离连续性的原因。"多头"指同时并行跑多套（如8套）注意力，每套关注不同维度的特征（如一套关注颜色相似性、一套关注方向连续性），最后拼合。MHSA之后的FFN则对每个位置的特征单独做一次非线性变换，让表达更有弹性。

**输入输出对比**

| | 内容 | 维度 | 作用 |
| --- | --- | --- | --- |
| 输入① | 压缩序列 S（Backbone特征图展平） | [B, 920, 256] | 携带每个位置的语义特征 |
| 输入② | 位置嵌入 Eₚ（正弦/余弦编码，固定不变） | [B, 920, 256] | 补回展平时丢失的空间坐标信息 |
| 实际输入 | **S + Eₚ**（直接相加） | [B, 920, 256] | 语义 + 位置，缺一不可 |
| 输出 | 全局关联序列 Sₑ | [B, 920, 256] | 维度不变，但每个点都融合了全图信息 |

**维度对比：** 输入 [B,920,256] → 输出 [B,920,256]，维度完全不变，但内容变了——输入时每个位置只有自身特征，输出时每个位置都融合了全图所有位置的关联信息。就像一群人开会，人数没变，但开完会后每个人掌握的信息量都变大了。

##### 4.3.2.3 Transformer 解码器（Decoder）：编码序列 + 初始查询 → 车道级特征

**一句话理解：** 派N个带着"空白任务单"的侦探，先互相协商不撞车（Self-Attention），再分别去Encoder的全局特征里检索属于自己的车道信息（Cross-Attention），最终每个侦探的任务单上都写满了对应车道的特征。

Decoder的核心组件是多层"自注意力（Self-Attention）+ 交叉注意力（Cross-Attention）+ FFN"，专门解决车道检测中"预测数量不确定""车道位置区分""关联图像特征"三大问题。

**核心设计——N个"侦探"：** Encoder给出的是920个图像特征点，但我们最终要的是N条车道线（如N=7）。Decoder的办法是派出N个侦探去图里找车道线：每个侦探出发时手里拿着一张空白任务单（全0矩阵，即初始查询S_q），背上还有一个工牌（学习位置嵌入E_LL），写着他负责哪个车道位置（左1、左2、中间、右1……）。

**两种注意力的分工：**

- **Self-Attention（侦探互相协商，避免重复）：** N个侦探先开内部会议互相区分——"我去找左1，你去找左2，他去找右边"，确保N个槽位各司其职、不会全挤在同一条车道线上。
- **Cross-Attention（侦探去图里找证据）：** 协商完后，每个侦探拿着自己的任务单去Encoder输出的920个特征点里检索与自己车道相关的信息，融合进任务单。这里Encoder输出Sₑ作为键（K）/值（V），侦探的任务单作为查询（Q）——查询去找Encoder的输出，这就是"交叉"的含义。

**两种位置嵌入别搞混：** Encoder的Eₚ是固定的（公式算出、训练不变），描述图像上每个像素坐标，好比地图经纬度；Decoder的E_LL是可学习的（随训练更新），描述每条车道的语义角色（左1/右2/中间），好比车道的名字。

**输入输出对比**

| | 内容 | 维度 | 对应类比 |
| --- | --- | --- | --- |
| 输入① | 初始查询 S_q（全0矩阵或随机张量） | [B, N, 256] | N个侦探的空白任务单 |
| 输入② | 学习位置嵌入 E_LL（可训练参数） | [B, N, 256] | 侦探的工牌（负责哪条车道） |
| 输入③ | Encoder输出 Sₑ | [B, H×W, 256] | 案件证据库（920个全局特征点） |
| 输出 | 车道级解码序列 S_d | [B, N, 256] | N份填满信息的任务单 |

N由场景预设（城市道路N=5，高速路N=7）。Self-Attention让N个侦探互相区分避免重复，Cross-Attention让每个侦探去证据库里检索属于自己车道的信息（Sₑ作为键K/值V，查询Q来自侦探任务单）。

**输出S_d里的N个元素代表什么？** 以N=7为例，7个元素分别对应"左3/左2/左1/中间/右1/右2"等潜在车道的特征；如果图里实际只有6条线，那第7个侦探没找到真实车道，其输出就是"背景特征"（对应原文"未对应真实车道的元素为背景特征"）。输入的N个空白任务单，出来时变成了N个填满车道信息的特征。

##### 4.3.2.4 车道线预测头（含FFNs）：解码序列 → 车道线曲线参数

**一句话理解：** 把N份"车道特征任务单"（抽象向量）翻译成能画在图上的具体曲线参数（数字）。

预测头由三部分组成：FFNs负责把抽象特征翻译成曲线参数，置信度过滤模块负责去掉没找到车道的"空手侦探"，匈牙利损失单元负责训练时的误差计算（推理时不参与）。

**FFNs：特征→曲线参数。** Decoder输出的每条车道是256维抽象特征，无法直接画到图上。FFN（小型全连接网络）把它浓缩成K维曲线参数（K远小于256，如K=8）。车道线在数学上可用多项式方程 x = aY² + bY + c 描述，其中a、b、c等系数加上起止点、置信度就是这K个数字。维度变化为 [B,N,256] → [B,N,K]。

**置信度过滤：去掉没找到车道的侦探。** FFN在输出曲线参数的同时也输出一个置信度分数（0~1）：≥0.5保留，<0.5丢弃。于是有效车道从N条筛成M条（M≤N），维度 [B,N,K] → [B,M,T]（T为每条线上的坐标点数，如50）。原文提到的"平滑插值"是因为光线差、有遮挡处某段车道的置信度会忽高忽低导致线条断续，取附近的置信度平均可让线条连续稳定。

**匈牙利损失（仅训练时用，推理时用不上）。** 训练时模型预测出N条候选、但真实标注可能只有4条，如何判断预测得准不准？用匈牙利算法做最优配对——把预测曲线和真实曲线两两匹配、找总误差最小的方案，再据此计算损失、反向更新模型。好比老师批卷时，先决定"哪个学生的答案对应哪道题"使总分最优，再打分。

**输入输出对比**

| 阶段 | 内容 | 维度 | 说明 |
| --- | --- | --- | --- |
| 输入 | 解码序列 S_d | [B, N, 256] | 来自Decoder，N条车道的抽象特征 |
| FFNs输出 | 曲线参数 H | [B, N, K] | K个数字描述一条车道线（如多项式系数a/b/c + 起止点 + 置信度） |
| 过滤后输出 | 最终车道线 R | [B, M, T] | M条有效车道（M≤N），每条T个坐标点，可直接画到图上 |

置信度≥0.5保留，<0.5丢弃，所以M≤N。匈牙利损失只在**训练时**参与（把预测曲线H和真实标注L做最优配对计算误差），推理时整个损失模块不参与。
...
📷  原图 [B, 3, 368, 640]
    ↓
🗺️  Backbone（ResNet）
    提取局部语义特征 → [B, 256, 23, 40]
    ↓ 展平
🔗  Encoder（多头自注意力 MHSA）
    920 个点两两对话，获取全局关联 → Sₑ [B, 920, 256]
    ↓
🔍  Decoder（Self-Attention + Cross-Attention）
    N 个 Query 去 Sₑ 里检索各自车道信息 → S_d [B, N, 256]
    ↓
📐  预测头 FFNs
    256 维特征 → K 维曲线参数 → 置信度过滤
    ↓
✅  最终输出 [B, M, T]
    M 条有效车道线，每条 50 个坐标点，可直接画到图上
```

三个最重要的设计思想：

1. **Backbone 负责"看"：** 用预训练经验把图片压缩成密集语义特征，不必从头学"什么是线条"。
2. **Encoder 负责"理解"：** 让所有位置互相交流，弥补 CNN 只能看局部的缺陷，车道线的连续性从这里来。
3. **Decoder 负责"分配"：** N 个 Query 就是 N 个预留槽位，端到端地从全局特征里直接"领取"各自车道的信息，不需要手工后处理。
#### 4.3.3 ONNX C++ 部署 LSTR
...
以下代码按执行阶段拆分，三段合并后组成完整的 LSTR::detect()。
