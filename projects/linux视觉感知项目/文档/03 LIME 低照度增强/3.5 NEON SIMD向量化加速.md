...
flowchart LR
    ...
    subgraph 第一次归约["vpaddq_f32"]
        direction LR
        P0["s0+s1"] ~~~ P1["s2+s3"] ~~~ P2["0"] ~~~ P3["0"]
    end
    subgraph 第二次归约["vpaddq_f32"]
        direction LR
        R0["s0+s1+s2+s3"] ~~~ R1["0"] ~~~ R2["0"] ~~~ R3["0"]
    end
    初始状态 --> 第一次归约 --> 第二次归约
```
...
快速傅里叶变换（FFT）的核心是**蝶形运算**——将 DFT 分解为逐级合并的子问题。对于长度为 N 的序列，蝶形运算的每一级都需要执行 N/2 次复数乘加操作。复数乘法的形式为：

```
(a + bi)(c + di) = (ac - bd) + (ad + bc)i
```

即一次蝶形运算需要 4 次实数乘法和 2 次实数加法。
