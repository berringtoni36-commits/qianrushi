...
created: "2026-08-13"
type: "reference"
summary: "小林coding《图解网络》官网目录第 4.23 TCP 四次挥手，可以变成三次吗？ 篇。"
source: "https://xiaolincoding.com/network/3_tcp/tcp_three_fin.html"
author: "小林coding"
...
<!-- WIKI_NAV_START -->
> [!info] 图解网络导航
> [[00｜图解网络目录|返回图解网络目录]] · [[4.22 用了 TCP 协议，数据一定不会丢吗？|← 上一篇：4.22 用了 TCP 协议，数据一定不会丢吗？]] · [[4.24 TCP 序列号和确认号是如何变化的？|下一篇：4.24 TCP 序列号和确认号是如何变化的？ →]]
<!-- WIKI_NAV_END -->


> [!info] 原文信息
> 原文：[4.23 TCP 四次挥手，可以变成三次吗？](https://xiaolincoding.com/network/3_tcp/tcp_three_fin.html)
> 提取：Defuddle 0.19.2
...
![](../../../../../assets/图解网络-小林coding/35-tcp_three_fin/img-001.png)

虽然我们在学习 TCP 挥手时，学到的是需要四次来完成 TCP 挥手，但是 **在一些情况下， TCP 四次挥手是可以变成 TCP 三次挥手的** 。

![在这里插入图片描述](../../../../../assets/图解网络-小林coding/35-tcp_three_fin/img-002.png)

而且在用 wireshark 工具抓包的时候，我们也会常看到 TCP 挥手过程是三次，而不是四次，如下图：

![在这里插入图片描述](../../../../../assets/图解网络-小林coding/35-tcp_three_fin/img-003.png)
...
![在这里插入图片描述](../../../../../assets/图解网络-小林coding/35-tcp_three_fin/img-004.png)
...
![在这里插入图片描述](../../../../../assets/图解网络-小林coding/35-tcp_three_fin/img-005.png)
...
![优雅关闭.drawio.png](../../../../../assets/图解网络-小林coding/35-tcp_three_fin/img-006.png)
...
![在这里插入图片描述](../../../../../assets/图解网络-小林coding/35-tcp_three_fin/img-007.png)
...
![](../../../../../assets/图解网络-小林coding/35-tcp_three_fin/img-008.png)

延迟等待的时间是在 Linux 内核中定义的，如下图：

![](../../../../../assets/图解网络-小林coding/35-tcp_three_fin/img-009.png)

关键就需要 HZ 这个数值大小，HZ 是跟系统的时钟频率有关，每个操作系统都不一样，在我的 Linux 系统中 HZ 大小是 1000，如下图：

![](../../../../../assets/图解网络-小林coding/35-tcp_three_fin/img-010.png)
...
如果要关闭 TCP 延迟确认机制，可以在 Socket 设置里启用 TCP\_QUICKACK。
...
![在这里插入图片描述](../../../../../assets/图解网络-小林coding/35-tcp_three_fin/img-011.png)

先启用服务端：

![在这里插入图片描述](../../../../../assets/图解网络-小林coding/35-tcp_three_fin/img-012.png)
...
![在这里插入图片描述](../../../../../assets/图解网络-小林coding/35-tcp_three_fin/img-013.png)

此时，服务端的输出：

![在这里插入图片描述](../../../../../assets/图解网络-小林coding/35-tcp_three_fin/img-014.png)

接下来，我们来看看抓包的结果。

![在这里插入图片描述](../../../../../assets/图解网络-小林coding/35-tcp_three_fin/img-015.png)
...
在上面服务端代码中，增加了打开了 TCP\_QUICKACK （快速应答）机制的代码，如下：

![在这里插入图片描述](../../../../../assets/图解网络-小林coding/35-tcp_three_fin/img-016.png)

编译好服务端代码后，就开始运行服务端和客户端的代码，同时用 tcpdump 进行抓包。

抓包的结果如下，可以看到是四次挥手。 ![在这里插入图片描述](../../../../../assets/图解网络-小林coding/35-tcp_three_fin/img-017.png)

所以，当被动关闭方（上图的服务端）在 TCP 挥手过程中，「 **没有数据要发送」，同时「关闭了 TCP 延迟确认机制」，那么就会是四次挥手。**

> 设置 TCP\_QUICKACK 的代码，为什么要放在 read 返回 0 之后？

我也是多次实验才发现，在 bind 之前设置 TCP\_QUICKACK 是不生效的，只有在 read 返回 0 的时候，设置 TCP\_QUICKACK 才会出现四次挥手。

网上查了下资料说，设置 TCP\_QUICKACK 并不是永久的，所以每次读取数据的时候，如果想要立刻回 ACK，那就得在每次读取数据之后，重新设置 TCP\_QUICKACK。

而我这里的实验，目的是为了当收到客户端的 FIN 报文（第一次挥手）后，立马回 ACK 报文。所以就在 read 返回 0 的时候，设置 TCP\_QUICKACK。当然，实际应用中，没人会在这个位置设置 TCP\_QUICKACK，因为操作系统都通过 TCP 延迟确认机制帮我们把四次挥手优化成了三次挥手了。
