---
title: "嵌入式 ARM Linux 系统架构全解：一文讲透硬件、U-Boot、内核、驱动与应用"
...
tags:
  ...
  - 系统架构
  - 面试八股
source: "https://mp.weixin.qq.com/s?__biz=MzkzNDk2NTUwOQ==&mid=2247491452&idx=1&sn=d059cfc6bc115a4accc95f02cd3260aa&chksm=c2b46716f5c3ee0070c567693a81b49a9b702968afa480a31daa4580f6e614735fa2014038b9"
---
...
> 作者：Debug 蟹老板
> 公众号：Linux教程
> 发布时间：2026年8月7日 20:31
> 原文：[微信公众号文章](https://mp.weixin.qq.com/s?__biz=MzkzNDk2NTUwOQ==&mid=2247491452&idx=1&sn=d059cfc6bc115a4accc95f02cd3260aa&chksm=c2b46716f5c3ee0070c567693a81b49a9b702968afa480a31daa4580f6e614735fa2014038b9)
...
```
┌────────────────────────────────────┐│        业务应用与后台服务           │├────────────────────────────────────┤│   C/C++ 运行库、BusyBox、系统服务   │├────────────────────────────────────┤│            根文件系统               │├────────────────────────────────────┤│ Linux 内核、子系统与设备驱动        │├────────────────────────────────────┤│      Device Tree 硬件描述           │├────────────────────────────────────┤│       U-Boot / SPL / 固件           │├────────────────────────────────────┤│ Boot ROM、SoC、DDR、Flash、外设     │└────────────────────────────────────┘
```

设备从上电到运行应用程序，大致会经历下面的流程：

```
设备上电   ↓电源和时钟稳定   ↓CPU 解除复位   ↓执行芯片内部 Boot ROM   ↓加载 SPL   ↓初始化 DDR   ↓加载完整 U-Boot   ↓加载 Linux 内核与设备树   ↓Linux 内核初始化   ↓加载设备驱动   ↓挂载根文件系统   ↓启动 PID 1   ↓启动系统服务   ↓运行产品应用程序
```
...
```
EL0：普通用户程序EL1：操作系统内核EL2：虚拟化管理程序EL3：安全监控固件
```
...
![[assets/embedded-interview/嵌入式 ARM Linux 系统架构全解：一文讲透硬件、U-Boot、内核、驱动与应用/img-001.jpg]]
...
```
┌─────────────────────────────────┐│ CPU Cluster                     │├─────────────────────────────────┤│ GPU / NPU / DSP / ISP           │├─────────────────────────────────┤│ DDR Controller / Interconnect   │├─────────────────────────────────┤│ GIC / Timer / DMA Controller    │├─────────────────────────────────┤│ UART / I²C / SPI / CAN / USB    │├─────────────────────────────────┤│ Ethernet / PCIe / SD / eMMC     │├─────────────────────────────────┤│ Clock / Reset / Power Domain    │└─────────────────────────────────┘
```
...
![[assets/embedded-interview/嵌入式 ARM Linux 系统架构全解：一文讲透硬件、U-Boot、内核、驱动与应用/img-002.jpg]]
...
```
CPU Core：约 0.8V～1.xVDDR：1.1V、1.2V 或其他电压I/O：1.8V 或 3.3V外设电源：3.3V、5V
```
...
```
BOOT_MODE = 000：eMMCBOOT_MODE = 001：SD 卡BOOT_MODE = 010：SPI NORBOOT_MODE = 011：USB 下载模式
```
...
![[assets/embedded-interview/嵌入式 ARM Linux 系统架构全解：一文讲透硬件、U-Boot、内核、驱动与应用/img-003.jpg]]
...
```
SoC I2C2_SCL ───── Sensor SCLSoC I2C2_SDA ───── Sensor SDASensor INT    ───── GPIO3_B2Sensor VCC    ───── 3V3_SENSOR
```

设备树中就可能需要描述：

```
&i2c2 {    status = "okay";    sensor@48 {        compatible = "vendor,temp-sensor";        reg = <0x48>;        interrupt-parent = <&gpio3>;        interrupts = <10 IRQ_TYPE_LEVEL_LOW>;        vdd-supply = <&vcc_3v3_sensor>;    };};
```
...
![[assets/embedded-interview/嵌入式 ARM Linux 系统架构全解：一文讲透硬件、U-Boot、内核、驱动与应用/img-004.jpg]]
...
```
应用程序   ↓ open/read/write/ioctl系统调用接口   ↓Linux 设备驱动   ↓MMIO / GPIO / I²C / SPI / DMA   ↓硬件设备
```

这样可以实现**权限隔离、资源共享和统一管理。**

![[assets/embedded-interview/嵌入式 ARM Linux 系统架构全解：一文讲透硬件、U-Boot、内核、驱动与应用/img-005.jpg]]
...
![[assets/embedded-interview/嵌入式 ARM Linux 系统架构全解：一文讲透硬件、U-Boot、内核、驱动与应用/img-006.jpg]]
...
```
128MB DDR16MB NOR Flash512MB NAND
```
...
![[assets/embedded-interview/嵌入式 ARM Linux 系统架构全解：一文讲透硬件、U-Boot、内核、驱动与应用/img-007.jpg]]
...
```
eFuse 配置   ↓启动模式引脚   ↓eMMC Boot Partition   ↓SD 卡   ↓SPI NOR   ↓USB Recovery
```
...
![[assets/embedded-interview/嵌入式 ARM Linux 系统架构全解：一文讲透硬件、U-Boot、内核、驱动与应用/img-008.jpg]]
...
![[assets/embedded-interview/嵌入式 ARM Linux 系统架构全解：一文讲透硬件、U-Boot、内核、驱动与应用/img-009.jpg]]
...
```
eMMCSDNANDSPI NORUSBTFTPNFSSATANVMe
```

典型过程为：

```
读取 Kernel Image 到 DDR读取 DTB 到 DDR读取 Initramfs 到 DDR设置 bootargs跳转到内核入口
```
...
```
md      查看内存mw      修改内存mmc     访问 eMMC/SDsf      访问 SPI Flashnand    访问 NANDtftp    网络下载ping    测试网络fdt     修改设备树booti   启动 ARM64 Imagebootz   启动 zImagebootm   启动 U-Boot 镜像或 FIT
```

![[assets/embedded-interview/嵌入式 ARM Linux 系统架构全解：一文讲透硬件、U-Boot、内核、驱动与应用/img-010.jpg]]
...
```
setenv bootcmd \'mmc dev 0; run load_kernel; run load_fdt; run boot_linux'
```
...
```
loadaddrkernel_addr_rfdt_addr_rramdisk_addr_r
```
...
![[assets/embedded-interview/嵌入式 ARM Linux 系统架构全解：一文讲透硬件、U-Boot、内核、驱动与应用/img-011.jpg]]
...
```
MMC init failedNo boot deviceBad magic
```
...
```
bdinfoprintenv kernel_addr_rprintenv fdt_addr_rprintenv ramdisk_addr_r
```
...
```
FDT_ERR_BADMAGICWrong Image FormatCould not find a valid device tree
```
...
![[assets/embedded-interview/嵌入式 ARM Linux 系统架构全解：一文讲透硬件、U-Boot、内核、驱动与应用/img-012.jpg]]
...
![[assets/embedded-interview/嵌入式 ARM Linux 系统架构全解：一文讲透硬件、U-Boot、内核、驱动与应用/img-013.jpg]]
...
```
内核入口   ↓建立早期页表   ↓切换异常级别和执行环境   ↓解压或重定位内核   ↓start_kernel()   ↓初始化内存管理   ↓初始化中断和定时器   ↓初始化调度器   ↓初始化驱动模型   ↓执行 initcall   ↓挂载根文件系统   ↓执行 PID 1
```
...
```
early_initcallcore_initcallpostcore_initcallarch_initcallsubsys_initcallfs_initcalldevice_initcalllate_initcall
```
...
![[assets/embedded-interview/嵌入式 ARM Linux 系统架构全解：一文讲透硬件、U-Boot、内核、驱动与应用/img-014.jpg]]
...
```
drivers/gpiodrivers/i2cdrivers/spidrivers/netdrivers/mmcdrivers/usbdrivers/pcidrivers/gpudrivers/media
```
...
![[assets/embedded-interview/嵌入式 ARM Linux 系统架构全解：一文讲透硬件、U-Boot、内核、驱动与应用/img-015.jpg]]
...
![[assets/embedded-interview/嵌入式 ARM Linux 系统架构全解：一文讲透硬件、U-Boot、内核、驱动与应用/img-016.jpg]]
...
![[assets/embedded-interview/嵌入式 ARM Linux 系统架构全解：一文讲透硬件、U-Boot、内核、驱动与应用/img-017.jpg]]
...
![[assets/embedded-interview/嵌入式 ARM Linux 系统架构全解：一文讲透硬件、U-Boot、内核、驱动与应用/img-018.jpg]]
...
![[assets/embedded-interview/嵌入式 ARM Linux 系统架构全解：一文讲透硬件、U-Boot、内核、驱动与应用/img-019.jpg]]
...
```
uart3: serial@30860000 {    compatible = "vendor,soc-uart";    reg = <0x0 0x30860000 0x0 0x10000>;    interrupts = <GIC_SPI 58 IRQ_TYPE_LEVEL_HIGH>;    clocks = <&clk UART3_CLK>;    status = "disabled";};
```
...
```
okaydisabledreserved
```

**`status = "disabled"` 的节点一般不会创建可供普通驱动绑定的设备。**

![[assets/embedded-interview/嵌入式 ARM Linux 系统架构全解：一文讲透硬件、U-Boot、内核、驱动与应用/img-020.jpg]]
...
```
static struct platform_driver demo_driver = {    .probe = demo_probe,    .remove = demo_remove,    .driver = {        .name = "demo-device",        .of_match_table = demo_of_match,    },};
```
...
```
void __iomem *base;int irq;base = devm_platform_ioremap_resource(pdev, 0);if (IS_ERR(base))    return PTR_ERR(base);irq = platform_get_irq(pdev, 0);if (irq < 0)    return irq;
```

还可以获取：

```
ClockResetGPIORegulatorDMA ChannelPHYIOMMU
```

![[assets/embedded-interview/嵌入式 ARM Linux 系统架构全解：一文讲透硬件、U-Boot、内核、驱动与应用/img-021.jpg]]
...
```
打开电源   ↓打开总线时钟   ↓打开功能时钟   ↓解除复位   ↓初始化寄存器
```
...
![[assets/embedded-interview/嵌入式 ARM Linux 系统架构全解：一文讲透硬件、U-Boot、内核、驱动与应用/img-022.jpg]]
...
```
应用程序   ↓系统调用或子系统接口   ↓Linux 驱动   ↓寄存器、中断、DMA、总线   ↓硬件设备
```
...
![[assets/embedded-interview/嵌入式 ARM Linux 系统架构全解：一文讲透硬件、U-Boot、内核、驱动与应用/img-023.jpg]]
...
```
/net/block/input/tty/video4linux
```
...
![[assets/embedded-interview/嵌入式 ARM Linux 系统架构全解：一文讲透硬件、U-Boot、内核、驱动与应用/img-024.jpg]]
...
![[assets/embedded-interview/嵌入式 ARM Linux 系统架构全解：一文讲透硬件、U-Boot、内核、驱动与应用/img-025.jpg]]
...
```
struct gpio_desc *reset_gpio;reset_gpio = devm_gpiod_get(dev,                            "reset",                            GPIOD_OUT_HIGH);
```
...
```
ret = devm_request_threaded_irq(dev,                                irq,                                demo_irq,                                demo_irq_thread,                                IRQF_ONESHOT,                                "demo",                                data);
```
...
```
clk_prepare_enable(clk);reset_control_deassert(rst);regulator_enable(vdd);
```
...
![[assets/embedded-interview/嵌入式 ARM Linux 系统架构全解：一文讲透硬件、U-Boot、内核、驱动与应用/img-026.jpg]]
...
```
.open.read.write.unlocked_ioctl.mmap.poll.release
```

应用程序调用：

```
fd = open("/dev/demo", O_RDWR);read(fd, buf, len);ioctl(fd, DEMO_START, &arg);
```
...
![[assets/embedded-interview/嵌入式 ARM Linux 系统架构全解：一文讲透硬件、U-Boot、内核、驱动与应用/img-027.jpg]]
...
![[assets/embedded-interview/嵌入式 ARM Linux 系统架构全解：一文讲透硬件、U-Boot、内核、驱动与应用/img-028.jpg]]
...
![[assets/embedded-interview/嵌入式 ARM Linux 系统架构全解：一文讲透硬件、U-Boot、内核、驱动与应用/img-029.jpg]]
...
```
inittabfstabpasswdnetworkinit.d
```
...
![[assets/embedded-interview/嵌入式 ARM Linux 系统架构全解：一文讲透硬件、U-Boot、内核、驱动与应用/img-030.jpg]]
...
```
/bin/ls   -> busybox/bin/cp   -> busybox/bin/sh   -> busybox/sbin/init -> ../bin/busybox
```
...
```
make menuconfigmake CROSS_COMPILE=aarch64-linux-gnu-make CONFIG_PREFIX=$PWD/rootfs install
```

然后创建：

```
rootfs/├── bin├── sbin├── etc├── dev├── proc├── sys├── lib├── usr├── var└── tmp
```

BusyBox 官方资料说明，被编译进 BusyBox 的命令以 Applet 形式存在，并可通过 BusyBox 本体或相应软链接进行调用。

![[assets/embedded-interview/嵌入式 ARM Linux 系统架构全解：一文讲透硬件、U-Boot、内核、驱动与应用/img-031.jpg]]
...
![[assets/embedded-interview/嵌入式 ARM Linux 系统架构全解：一文讲透硬件、U-Boot、内核、驱动与应用/img-032.jpg]]
...
```
SquashFS 只读系统        +可写 Data 分区        ↓OverlayFS 统一视图
```

适合只读根文件系统以及恢复出厂设置。

![[assets/embedded-interview/嵌入式 ARM Linux 系统架构全解：一文讲透硬件、U-Boot、内核、驱动与应用/img-033.jpg]]
...
![[assets/embedded-interview/嵌入式 ARM Linux 系统架构全解：一文讲透硬件、U-Boot、内核、驱动与应用/img-034.jpg]]
...
```
BusyBox initSysVinitsystemdOpenRC init自定义 init
```
...
![[assets/embedded-interview/嵌入式 ARM Linux 系统架构全解：一文讲透硬件、U-Boot、内核、驱动与应用/img-035.jpg]]
...
![[assets/embedded-interview/嵌入式 ARM Linux 系统架构全解：一文讲透硬件、U-Boot、内核、驱动与应用/img-036.jpg]]
...
```
#!/bin/shmount -t proc proc /procmount -t sysfs sysfs /sysmount -t devtmpfs devtmpfs /devhostname embedded-deviceifconfig lo upifconfig eth0 upmkdir -p /var/runmkdir -p /var/log/etc/init.d/S40network start/etc/init.d/S50sshd start/etc/init.d/S90application start
```

**BusyBox 的 `inittab` 语法具有自身特点，不应直接套用其他 init 实现的配置格式。**

![[assets/embedded-interview/嵌入式 ARM Linux 系统架构全解：一文讲透硬件、U-Boot、内核、驱动与应用/img-037.jpg]]
...
![[assets/embedded-interview/嵌入式 ARM Linux 系统架构全解：一文讲透硬件、U-Boot、内核、驱动与应用/img-038.jpg]]
...
![[assets/embedded-interview/嵌入式 ARM Linux 系统架构全解：一文讲透硬件、U-Boot、内核、驱动与应用/img-039.jpg]]
...
![[assets/embedded-interview/嵌入式 ARM Linux 系统架构全解：一文讲透硬件、U-Boot、内核、驱动与应用/img-040.jpg]]
...
```
open();read();write();ioctl();socket();mmap();
```
...
```
/dev/ttyS0/dev/i2c-1/dev/spidev0.0/dev/video0/dev/input/event0
```
...
![[assets/embedded-interview/嵌入式 ARM Linux 系统架构全解：一文讲透硬件、U-Boot、内核、驱动与应用/img-041.jpg]]
...
![[assets/embedded-interview/嵌入式 ARM Linux 系统架构全解：一文讲透硬件、U-Boot、内核、驱动与应用/img-042.jpg]]
...
![[assets/embedded-interview/嵌入式 ARM Linux 系统架构全解：一文讲透硬件、U-Boot、内核、驱动与应用/img-043.jpg]]
...
```
ssize_t ret = read(fd, buf, sizeof(buf));if (ret < 0) {    if (errno == EINTR)        ...}
```
...
![[assets/embedded-interview/嵌入式 ARM Linux 系统架构全解：一文讲透硬件、U-Boot、内核、驱动与应用/img-044.jpg]]
...
**x86_64 编译器产生的机器代码不能直接在 ARM CPU 上运行。**
...
```
Build Host：x86_64 PCTarget：ARM 开发板
```
...
```
aarch64-linux-gnu-gccaarch64-linux-gnu-g++aarch64-linux-gnu-ldaarch64-linux-gnu-asaarch64-linux-gnu-objcopyaarch64-linux-gnu-stripaarch64-linux-gnu-gdb
```
...
```
arm-linux-gnueabiarm-linux-gnueabihf
```
...
![[assets/embedded-interview/嵌入式 ARM Linux 系统架构全解：一文讲透硬件、U-Boot、内核、驱动与应用/img-045.jpg]]
...
```
make ARCH=arm \     CROSS_COMPILE=arm-linux-gnueabihf- \     board_defconfigmake ARCH=arm \     CROSS_COMPILE=arm-linux-gnueabihf- \     -j$(nproc)
```

### 编译 ARM64 Linux 内核

```
make ARCH=arm64 \     CROSS_COMPILE=aarch64-linux-gnu- \     board_defconfigmake ARCH=arm64 \     CROSS_COMPILE=aarch64-linux-gnu- \     -j$(nproc) \     Image dtbs modules
```
...
```
make ARCH=arm64 \     CROSS_COMPILE=aarch64-linux-gnu- \     menuconfigmake ARCH=arm64 \     CROSS_COMPILE=aarch64-linux-gnu- \     -j$(nproc)make ARCH=arm64 \     CROSS_COMPILE=aarch64-linux-gnu- \     CONFIG_PREFIX=$PWD/rootfs \     install
```

### 编译应用程序

```
aarch64-linux-gnu-gcc \    main.c \    -o application
```

然后把程序以及依赖库复制到根文件系统。

![[assets/embedded-interview/嵌入式 ARM Linux 系统架构全解：一文讲透硬件、U-Boot、内核、驱动与应用/img-046.jpg]]
...
```
U-BootLinux Kernel设备树驱动补丁交叉工具链Buildroot/Yocto烧录工具多媒体库NPU/GPU SDK示例程序
```

使用厂商 SDK 可以更快让硬件工作，但是也要避免过度依赖未经整理的大量私有补丁。

![[assets/embedded-interview/嵌入式 ARM Linux 系统架构全解：一文讲透硬件、U-Boot、内核、驱动与应用/img-047.jpg]]
...
```
┌──────────────────────┐│ Bootloader / SPL     │├──────────────────────┤│ Environment          │├──────────────────────┤│ Boot / Kernel / DTB  │├──────────────────────┤│ RootFS A             │├──────────────────────┤│ RootFS B             │├──────────────────────┤│ Recovery             │├──────────────────────┤│ Data                 │└──────────────────────┘
```
...
![[assets/embedded-interview/嵌入式 ARM Linux 系统架构全解：一文讲透硬件、U-Boot、内核、驱动与应用/img-048.jpg]]
...
```
dd if=system.img \   of=/dev/sdX \   bs=4M \   conv=fsync
```
...
```
tftp ${loadaddr} Imagetftp ${fdt_addr_r} board.dtb
```
...
```
minicompicocomscreenPuTTYMobaXterm
```
...
```
perf topperf record -g ./applicationperf report
```
...
```
topfree -mvmstat 1iostatpidstat
```

### 网络工具

```
tcpdumpethtoolssippingiperf3
```
...
```
当前运行 Slot A       ↓下载更新到 Slot B       ↓校验签名和 Hash       ↓设置下次启动 Slot B       ↓重启并试运行       ↓应用上报启动成功       ↓确认 Slot B
```
...
```
Boot ROM 公钥根      ↓验证 SPL      ↓SPL 验证 U-Boot      ↓U-Boot 验证 Kernel/FIT      ↓内核验证 RootFS 或应用
```
...
![[assets/embedded-interview/嵌入式 ARM Linux 系统架构全解：一文讲透硬件、U-Boot、内核、驱动与应用/img-049.jpg]]
...
```
硬件上电   ↓Boot ROM   ↓SPL 初始化 DDR   ↓U-Boot 加载系统   ↓Linux 内核管理资源   ↓设备树描述硬件   ↓驱动控制设备   ↓根文件系统提供用户环境   ↓PID 1 启动服务   ↓应用程序实现业务
```
...
```
第一步：理解 ARM 硬件和启动过程第二步：掌握 U-Boot 与内核启动参数第三步：学会配置和编译 Linux 内核第四步：理解设备树和驱动匹配第五步：掌握常见总线与设备驱动第六步：构建 BusyBox 根文件系统第七步：学习应用开发和系统服务第八步：使用 Buildroot 或 Yocto 自动构建第九步：研究 OTA、安全启动和量产可靠性
```
