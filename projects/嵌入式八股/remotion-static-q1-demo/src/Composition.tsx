...
const PACING = {
  opening: { duration: 25, prompt: 8 },
  roles: { duration: 20 },
  local: { duration: 75, prompt: 10, declaration: 12, call: 14, recap: 11 },
  file: { duration: 45, prompt: 8, reveal: 10, test: 12, recap: 15 },
  member: { duration: 50, prompt: 8, explanation: 12, object: 10, recap: 10 },
  memory: { duration: 65, prompt: 10, basic: 8, data: 15, bss: 15, recap: 17 },
  followups: { duration: 80, each: 20, prompt: 8 },
  final: { duration: 30, each: 7.5, prompt: 5 },
} as const;
...
const fadeSlide = (frame: number, start: number, distance = 18) => ({
  opacity: clamp(frame, [start, start + 24], [0, 1]),
  translate: `0px ${clamp(frame, [start, start + 24], [distance, 0])}px`,
});
...
const MemoryScene: React.FC = () => {
  ...
  const prompt = frame < promptEnd;
  const stackActive = frame >= promptEnd && frame < promptEnd + seconds(6);
  const heapActive = frame >= promptEnd + seconds(6) && frame < basicEnd;
...
const FOLLOWUPS = [
  ...
  [
    "static 成员变量能在类内初始化吗？",
    "只有 const 整型或枚举等特殊情况可以；普通静态成员通常需要类外定义。",
    COLORS.compare,
  ],
] as const;
  ...
  return (
    <AbsoluteFill>
      ...
      <SceneTitle
        ...
        title="先回答，再看标准说法"
        detail="每个追问都留出 8 秒，再给 12 秒读答案和复述。"
      />
      <div
        style={{
          position: "absolute",
