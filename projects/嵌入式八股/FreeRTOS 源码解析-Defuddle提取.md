...
- [[#我把RTOS部分的八股复制了一份到这了，然后有知识库的同学就直接看下面的源码就OK了|导读]]
...
- [[#6. PendSv|6. PendSv]]
- [[#7. 上下文切换|7. 上下文切换]]
- [[#8. 内存堆栈管理和管理算法（heap1-heap5）（最好看一下源码）（掌握）|8. 内存堆栈管理]]
- [[#9. 任务间通信（必考）|9. 任务间通信]]
...
- [[#12. 延伸|12. 延伸]]
  - [[#第一章 list|第一章 list]]
  - [[#第二章 内存管理|第二章 内存管理]]
  - [[#第三章 task.c|第三章 task.c]]
  - [[#第四章 queue.c|第四章 queue.c]]
  - [[#第五章 timer.c|第五章 timer.c]]
  - [[#第六章 event_groups.c|第六章 event_groups.c]]

## 我把RTOS部分的八股复制了一份到这了，然后有知识库的同学就直接看下面的源码就OK了

> ‼️
>
> （先看快速入门手册做几个 demo（就是调用 api）知道咋回事之后再重点看视频内部机制！)
...
>
> 这个的话推荐先看一下韦东山老师的 PDF，就 174 页，很快就能翻完，知道这个概念就行，重点掌握抢占式任务调度、内存管理、消息队列、锁
>
> 建议后面阅读 RTOS 源码：需要掌握如结构体、指针、数组、函数指针、指针函数、数组指针、指针数组等这些基础知识点。
>
> 其次，一 些常用的数据结构要掌握，如链表、队列等。
>
> 怎么看 RTOS 源码？先看 Heap1-5，然后再去看后面的
>
> 第一阶段：攻克基础设施 —— list.c （必经之路）
>
> 在看任务调度之前，你必须、绝对、一定要先看 list.c 和 list.h。
>
> - 为什么？ FreeRTOS 的核心是“链表”。任务状态（就绪、阻塞、挂起）的管理，本质上就是把代表任务的结构体（TCB）从“就绪链表”挪到“阻塞链表”的过程。 如果不理解 list.c，你在 tasks.c 里看到各种 vListInsert、pxNext 时会一脸懵逼。
>
> - 看什么？
>
>   - FreeRTOS 用的是双向循环链表。
>
>   - 重点看 vListInsertEnd（尾插法，最常用）和 vListInsert（按值排序插入，用于延时列表）。
>
> - 目标： 你能手画出一个包含 3 个节点的 FreeRTOS 链表结构图，就算过关。
>
> 第二阶段：任务的诞生 —— xTaskCreate
>
> 现在回到 tasks.c，但不要试图通读全篇。我们只看“生孩子”的过程。
>
> - 结合你刚学的 Heap 知识： 你会发现 xTaskCreate 内部调用了 prvAllocateTCBAndStack。
>
>   - 这里就在用 pvPortMalloc（你熟悉的 Heap 知识）！
>
>   - 它分配了两块肉：一块给 TCB（任务控制块，任务的身份证），一块给 Stack（任务的栈，任务的草稿纸）。
>
> - 关键结构体 TCB_t：在 tasks.c 里找到 tskTaskControlBlock 的定义。
>   - 不需要背下来，但要看懂里面的 xStateListItem —— 这就是用来把任务挂到第一阶段那个链表上的“钩子”。
>
> 第三阶段：核心机制 —— 调度器启动与切换（难点）
> 这是最硬的骨头，分为两部分：C 语言部分和汇编部分。
> 1. 调度器启动（vTaskStartScheduler）：
>   - 看它如何创建“空闲任务（Idle Task）”。
>   - 看它如何配置 SysTick 定时器（操作系统的“心跳”）。
> 2. 上下文切换（Context Switch）：
>   - 这里涉及 port.c 和 portasm.s（如果是在 ARM 上）。
>   - 核心逻辑：当发生切换时，CPU 需要把当前任务的 R0-R15 寄存器保存到该任务的栈里，然后从下一个任务的栈里把寄存器恢复出来。
>   - 建议：作为小白，不要深究汇编指令的每一个细节（如 STMDB，LDMIA），只需要理解“保存现场”和“恢复现场”的概念即可。
>
> 第四阶段：万物皆队列 —— queue.c
> FreeRTOS 的设计非常精妙，信号量（Semaphore）和互斥量（Mutex）其实都是队列（Queue）的“换皮版”。
> - 看什么？
>   - xQueueGenericSend（入队）和 xQueueGenericReceive（出队）。
>   - 重点中的重点：当队列满/空时，任务是如何进入“阻塞态”的？
>     - 你会看到它把任务从“就绪列表”移除，放到了“延时列表”或“事件等待列表”中。这里会把你第一阶段（链表）和第三阶段（调度）的知识全部串起来！
...
3. `FreeRTOS/portable/MemMang` 文件下：建议把五个内存管理的都移植过来
   - `FreeRTOS/portable/RVDS` 文件下：建议全部都移植过来，因为他是适配不同的内核的，我们常用的 M3、M4、M7 都有，我们需要的是 r0p1 里面的 port.c 和 portmacro.h。我是用的 H7 的开发板。
...
②编译完会发现有报错，不要慌，我们去 it.c 文件把这些对应的中断注释掉再编译就 ok 啦。

注：有的小伙伴会缺少 FreeRTOSConfig.h 文件，这个不用自己创建哈，建议去别的工程文件，就是我们下载的压缩包里面就有哦，在 example 文件夹下复制一个放到前面的 include 文件夹下就 ok 啦！FreeRTOSConfig.h 是何方神圣？看名字就知道，他是 FreeRTOS 的配置文件，一般的操作系统都有裁剪、配置功能，而这些裁剪及配置都是通过一个文件来完成的，基本都是通过宏定义来完成对系统的配置和裁剪的

③有些 uu 还会遇到一些未定义的函数，这个很简单，是因为自己复制的 FreeRTOSConfig.h 中开启了对应的钩子函数，直接去关闭这些钩子函数即可，将相应的宏定义设置为 0 即可。
...
先看一下内存分区吧，这个图最好掌握并且熟记（主要是内存分区，代码区、。data.bss、堆、栈）
...
1. 栈的初始化就是栈指针 SP 的初始化！ARM 处理器是使用 R13 寄存器和 R11 寄存器来管理堆栈。
...
> 5.
>
> 大小有限：栈的大小通常有限制（如几 MB），由操作系统和编译器决定。
>
> 6.
>
> 函数传参：ARM 处理器遵循 ATPCS 规则优化参数传递：前 4 个参数通过 R0-R3 寄存器高效传递，超出部分则压入栈中，实现寄存器与栈的协同传参机制。

#### ATPCS 核心规则（效率优先）

- 前 4 个参数：用寄存器 R0～R3 直接传递，速度最快
- 第 5 个起：压入调用者栈帧，遵循从右到左顺序（如参数 5、6 依次压栈）
  - 被调函数：需将寄存器参数转存至自身栈帧，栈参数则通过 FP 向后偏移直接读取
...
形参仅在函数调用时于栈帧内临时分配空间接收实参值的副本，函数返回后随栈帧销毁而释放。由于形参与实参存储在栈中不同的内存单元，二者完全隔离，修改形参仅是改写副本，永远无法影响实参原值。这一机制精准定义了局部变量的生命周期（函数执行期）与作用域（仅限本函数栈帧内），是理解 C 语言值传递本质与栈内存管理的核心。
...
如果栈的容量设置得过大，则会增加内存开销和启动时间；设置得过小，则程序超出栈设置的内存空间又容易发生栈溢出（Stack Overflow），产生段错误。

1. 栈溢出：（常问：如何去快速分配一个任务的堆栈大小？怎么监测？)
1. 栈的大小有限，如果局部变量占用过多内存（如递归调用过深或定义了过大的局部数组），可能会导致栈溢出，进而导致程序崩溃。

在设置栈大小时，我们要根据程序中的变量、数组对栈空间的实际需求，设置合理的栈大小。用户在编写程序时，为了防止栈溢出，可以
...
前面提到过，一个函数内定义的局部变量、传递的实参都是保存在栈中的。每一个函数都会有自己专门的栈空间来保存这些数据，每个函数的栈空间都被称为栈帧（Frame Pointer，FP）。

那么怎么去找到这个函数的栈帧呢？我们就需要用到 SP 指针和 FP 指针，也就是 R13 寄存器和 R11 寄存器。

![[assets/feishu-freertos-source-analysis/img-022.jpg]]

![[assets/feishu-freertos-source-analysis/img-023.jpg]]

我们在main()函数中调用了f()函数，main()函数的栈帧基址FP、main()函数的返回地址LR，都需要保存在f()函数的栈帧中。当f()函数运行结束退出时就可以根据栈中保存的地址返回函数的上一级继续

执行。

> 🌈
>
> 在多级函数调用中，每个函数拥有独立的栈帧空间，SP 指针如实时追踪的"吊臂"始终精准指向当前函数栈顶并随压栈/出栈动态变化，而 FP 指针则如"楼层基准牌"稳定指向栈帧基准位置（靠近栈底）以形成调用链、支撑调试回溯和相对寻址，二者协同实现层级隔离与自动管理，确保无论调用多深，CPU 总能唯一定位当前执行上下文。
>
...
>
> （4）bss 段：放置未被显式初始化或被显式初始化为 0 的全局变量和静态变量。

有些特殊数据会被放到代码段

> 📌
>
> （1）C 语言中使用 char \*p = “freertos”定义字符串指针时，字符串”freertos”实际被分配在代码段，也就是说它是一个常量字符串。
>
> （2）const 型常量：const 的实现方法至少有两种，第一种就是编译器将 const 常量放在代码段实现不能被修改（普遍见于各种单片机编译器）；第二种就是由编译器来检查以确保 const 常量不会被修改，实际上它是存放在数据段中的（gcc）。

总结：C 语言中所有变量和常量所使用的内存无非以上三种情况，栈、堆、全局（静态）区。
...
>
> 调度单位：进程是操作系统调度的基本单位，操作系统通过进程调度算法来分配 CPU 时间。
>
...
任务调度其实就是把 TCB 里的 ListItem，从一个 List 挪到另一个 List 而已！
...
你可以通过软件设置触发（写 SCB->ICSR 的 PENDSVSET 位），不会被硬件自动触发。
...
通常设置为所有异常中最低优先级，以确保只有当没有其他中断时才执行任务切换。

3.

软件触发

通常由 portYIELD() 或 xTaskSwitchContext() 内部触发：

SCB->ICSR |= SCB\_ICSR\_PENDSVSET\_Msk;
...
主要用于任务上下文切换（context switch）。

当调度器决定要从任务 A 切换到任务 B 时，会触发 PendSV 异常，让 CPU 进入异常处理模式，在其中：
...
> 2. PendSV 被触发
>
> 当 CPU 空闲（没有更高优先级中断）时，进入 PendSV\_Handler。
>
> 3. PendSV\_Handler 执行（汇编实现）（了解）
>
> \_\_asm void xPortPendSVHandler(void)
>
> {
>
...
> cpsid i
>
> bl vTaskSwitchContext // 调用 C 函数，选择下一个任务
>
> cpsie i
>
> pop {lr}
...
- PendSV：真正执行上下文切换（保存、恢复寄存器）。 简单来说： 🕒 SysTick 说“该切换任务了” ⚙️ PendSV 说“好，我来切换”
...
【【FreeRTOS】动画搞懂任务切换到底是怎么回事！】https://www.bilibili.com/video/BV1ErWyziEtw?vd\_source=47cd0923abff23e1bf03ec7d4aebca4c
...
>
> - SCB->ICSR |= SCB\_ICSR\_PENDSVSET\_Msk; // 触发 PendSV
>
> 2 进入 PendSV\_Handler（汇编级）
>
> 在异常入口：
>
> - 保存当前任务上下文
>
> - mrs r0, psp; 取出当前任务的堆栈指针
>
> stmdb r0!, {r4-r11}; 保存R4~R11到任务堆栈
>
> str r0, \[pxCurrentTCB\]; 更新TCB中记录的任务堆栈位置
>
> 3 调用调度函数
>
...
## 8. 内存堆栈管理和管理算法（heap1-heap5）（最好看一下源码）（掌握）
...
### 8.1 RTOS 自带的内存管理算法：（部分内容来自百问网（便于理解））
...
>
> 每个任务都独立维护自己的栈空间，栈空间总量在任务创建时进行设定。 uxTaskGetStackHighWaterMark（）主要用来查询指定任务的运行历史中，其栈空间还差
>
...
>   - 每个任务拥有独立、固定大小的栈。
>
>   - 高水位线是指任务自运行以来，栈空间剩余的最小值（以字为单位）。
>
>   - 这个值越小，说明栈曾经使用得越满；值为 0 表示栈曾完全耗尽；值为负数（当检查溢出时可能返回 pdSTACK\_GROWTH 相关的负值）或接近 0 表示栈已溢出。
>
> 使用方法：
>
> - 在开发调试阶段，周期性或事件触发地调用此函数，可以了解每个任务的栈需求峰值，从而合理设置 configMINIMAL\_STACK\_SIZE 或创建任务时指定的栈大小，并预留安全余量（例如，高水位线至少保留 10-20% 的栈空间）。
...
>
> - 钩子函数：void vApplicationStackOverflowHook(TaskHandle\_t \*pxTask, char \*pcTaskName);
>
>   - 功能：当检测到栈溢出时由内核自动调用。
>
...
#### 9.1.1 Queue（队列）FreeRTOS 中所有的通信与同步机制都是基于队列实现的！！！

1. （这个地方看一下队列的底层实现还有读写队列的函数实现）（信号量的本质其实就是队列，所以看队列的就足够了）（韦东山老师也有讲）
...
xQueueReceive(queue, &recvData, portMAX_DELAY); // 接收数据
```
...
>
>   - 互斥量（Mutex）：特殊二值信号量，用于共享资源保护，支持 优先级继承。
...
> 任务安全性
>
>   - 不要在多个任务直接访问同一全局变量，必须使用队列、互斥量或任务通知。
>
> 2.
>
> 阻塞与非阻塞
...
> 中断安全
>
>   - FreeRTOS 提供 FromISR 版本的 API（如 xQueueSendFromISR）用于中断中发送/接收数据。
>
> 4.
>
> 优先级注意
>
>   - 信号量和互斥量可能涉及 优先级继承，防止优先级反转问题。

#### 9.1.6 互斥量（必考）（这个底层实现也去看一下）
...
> ✍️
>
> 互斥量的一个重要特性是优先级继承（Priority Inheritance）。当高优先级任务等待低优先级任务持有的互斥量时，RTOS 会临时提升低优先级任务的优先级，使其尽快释放互斥量。这样可以避免优先级反转问题，确保系统的实时性。（这个地方可以去看一下源码！！！)
...
>
> 互斥量的一个重要特性是优先级继承（Priority Inheritance）。当高优先级任务等待低优先级任务持有的互斥量时，RTOS 会临时提升低优先级任务的优先级，使其尽快释放互斥量。这样可以避免优先级反转问题，确保系统的实时性。
...
>
> 此宏定义用于配置 STM32 的 8 位优先级设置寄存器实际使用的位数。STM32F103，STM32F407 和 STM32F429 都是使用的 4 位。
>
...
>
> #define configLIBRARY\_MAX\_SYSCALL\_INTERRUPT\_PRIORITY 0x01
>
> 此宏定义比较重要，定义了受 FreeRTOS 管理的最高优先级中断。简单的说就是允许用户在这个中断服务程序里面调用 FreeRTOS 的 API 的最高优先级。
...
RTOS低功耗设计原理及实现\_TicklessMode(FreeRTOS的实现).pdf
...
> 4.
>
> 调用汇编指令（如 WFI）让 CPU 进入低功耗模式（Stop 或 Sleep）。
>
> 5.
>
...
如果你能描述出具体的实现流程，面试官会觉得你有实战经验！！！

#### 10.2.1 默认实现 （基于 SysTick， 仅 Sleep 模式）

FreeRTOS 默认的 port.c 已经实现了基于 SysTick 的 Tickless。它只能进入 Sleep Mode （CPU 停，外设不停）。

- 优点： 开箱即用。
- 缺点： 功耗降低有限（mA 级别）。

#### 10.2.2 进阶实现 （基于 LPTIM， Stop 模式）

为了达到 uA 级别的功耗，必须进入 Stop Mode。这需要自定义钩子函数。
...
{
    // 1. 停止 SysTick
    // 2. 配置 LPTIM (低功耗定时器) 在 xExpectedIdleTime 后中断
    LPTIM_Config(xExpectedIdleTime);
    LPTIM_Start();
    ...
    // 5. 唤醒后，恢复系统时钟
    SystemClock_Config(); // 从 Stop 唤醒通常需要重配时钟

    // 6. 计算睡了多久
    uint32_t ulCompletedTicks = LPTIM_GetCounter();
...
> 🌈
>
> - 时钟恢复： 从 Stop 模式唤醒后，STM32 等芯片通常会默认使用内部低速时钟 （HSI），需要手动重新配置为高速时钟 （HSE/PLL），否则系统运行速度变慢。
>
> - 调试器断连： 进入低功耗模式会导致 J-Link/ST-Link 断开连接。调试技巧： 在低功耗代码前加个 if (DEBUG\_MODE) return; 或者在开发阶段暂时注释掉 WFI 指令。
>
> - 临界区保护： 在配置休眠前，必须进入临界区 （taskENTER\_CRITICAL），防止在配置定时器的过程中被中断打断，导致逻辑错误。
...
//配置后，当系统进入空闲任务（Idle Task）且预计空闲时间大于设定值时，FreeRTOS 会自动调用 portSUPPRESS_TICKS_AND_SLEEP( xExpectedIdleTime ) 函数。
```
...
1. 定时器到期唤醒： 此时经过的时间就是预设的 xExpectedIdleTime，系统直接将 OS tick 加上这个值。
1. 外部中断唤醒 （IRQ）： 比如按下按键。此时定时器还没到期。系统需要读取硬件定时器的当前计数值，计算出实际睡了多久，然后调用 vTaskStepTick() 更新系统时钟，保证系统时间准确。
...
- xEventGroupClearBitsFromISR() - 中断中清除
...
### 时间紧急就先看流程图了解逻辑还有思路最重要！

### 怎么去看 RTOS 的源码呢？前提是对于RTOS有一定的了解，可以速通一下韦东山老师的RTOS入门PDF，或者实时内核使用指南
...
- 精度与延迟：频率越高（如 1000Hz），任务调度的时间精度越高，任务的最大延迟越小（最多 1ms）
...
- 启用时（1）：相同优先级的多个任务会轮流执行，每个任务执行一个"时间片"（通常等于一个节拍周期，如 1ms）
- 禁用时（您的配置 0）：相同优先级的任务中，先就绪的任务会一直执行，直到：
  - 主动阻塞（如调用 vTaskDelay()）
  - 被更高优先级任务抢占
...
- 作用：控制是否使用针对特定硬件平台优化的任务选择算法。
  ...
  - 1 = 启用，使用平台特定的优化方法（通常更快，依赖硬件支持，如位操作指令）。
- 适用场景：如果硬件平台支持（如 ARM Cortex-M 系列的位带操作），启用可提升任务切换性能；否则保持 0。

### 低功耗 tickless 模式

![[assets/feishu-freertos-source-analysis/img-042.jpg]]
- 作用：控制是否启用低功耗 tickless 模式。
  - 0 = 保持 tick 中断一直运行（不进入低功耗模式）。
  - 1 = 启用 tickless 模式，空闲时可关闭 tick 中断以降低功耗。
...
- TICK\_TYPE\_WIDTH\_64\_BITS = 64 位无符号整数。
- 可选值还有 TICK\_TYPE\_WIDTH\_32\_BITS（32 位无符号整数）。
- 适用场景：
...
- 作用：定义用于存储任务栈深度的类型（如 xTaskCreate 等函数中的栈大小参数类型）。
- 影响：
  - size\_t 是 C 标准类型，通常与处理器字长匹配（32 位系统为 4 字节，64 位系统为 8 字节）。
  - 选择合适的类型可确保栈大小参数能正确表示实际需要的栈空间（避免溢出或类型不匹配）。
...
- 注意：
  - 新应用推荐使用 vTaskListTasks 和 vTaskGetRunTimeStatistics API，这些 API 会显式要求传入缓冲区长度，避免内存溢出。
  - 此配置仅对旧版统计 API 有效。
...
- 值：0
- 作用：控制是否启用 Newlib C 标准库的可重入支持。
  - 0 = 禁用，不使用 Newlib 的可重入版本（适用于不依赖 Newlib 或无需多任务同时调用 C 标准库的场景）。
  - 1 = 启用，使用 Newlib 的可重入版本（允许多任务同时调用 C 标准库函数，如 malloc、printf 等）。
- 注意：
  - Newlib 是嵌入式系统中常用的 C 标准库实现。
  - 启用时需确保系统已正确配置 Newlib 的线程安全支持（如提供 \_\_malloc\_lock/\_\_malloc\_unlock 等钩子函数）。
...
- 值：(configMAX\_PRIORITIES - 1)
  - 结合之前的配置 configMAX\_PRIORITIES = 5，实际优先级为 4（最高优先级，因为 FreeRTOS 中优先级 0 最低）。
- 作用：设置定时器守护任务的优先级。
- 影响：
...
- 影响：
  - 堆大小决定了可动态创建的任务、队列、信号量等对象的总内存上限。
  - 值过小可能导致 pvPortMalloc 失败（动态创建对象时返回 NULL）。
  - 值过大则会占用过多系统内存（需根据实际硬件资源调整）。

### 堆内存是否由应用程序预分配

![[assets/feishu-freertos-source-analysis/img-065.jpg]]
- 0 = 禁用，堆内存由 FreeRTOS 内核自动分配（使用 configTOTAL\_HEAP\_SIZE 定义大小）。
- 1 = 启用，堆内存由应用程序提供（需通过 vPortDefineHeapRegions 函数指定内存区域）。

### 是否从独立堆分配任务栈

![[assets/feishu-freertos-source-analysis/img-066.jpg]]
- 0 = 禁用，任务栈从主堆分配（与其他对象共用同一堆）。
- 1 = 启用，任务栈从独立堆分配（需实现 pvPortMallocStack/vPortFreeStack 函数）。

### 是否启用堆保护（检测堆溢出）

![[assets/feishu-freertos-source-analysis/img-067.jpg]]
- 0 = 禁用，不进行堆保护（可节省内存和性能）。
- 1 = 启用，在堆块前后添加保护区域，检测越界访问（增强稳定性，适用于调试阶段）。
...
- 影响：
  - 优先级高于此值的中断不能调用 FreeRTOS API（可能导致系统崩溃）。
  - 优先级低于或等于此值的中断可以调用 FreeRTOS API（如 xQueueSendFromISR）。
- 注意：
  - 此值通常应设置为比 configKERNEL\_INTERRUPT\_PRIORITY 更低的优先级，确保内核中断不受用户中断影响。
...
### 覆盖 MPU 中 Flash 区域的 TEX、S（可共享）、C（可缓存）、B（可缓冲）位的默认值（仅用于 Cortex-M MPU 移植）M7 内核会带 MPU
...
- 1 = 启用，内核自动为空闲任务（Idle Task）和定时器任务（Timer Task）分配静态内存（无需应用实现内存分配函数）。
- 0 = 禁用，应用需自行实现 vApplicationGetIdleTaskMemory() 和 vApplicationGetTimerTaskMemory() 函数来提供内存。
- 优势：简化应用代码，减少内存配置错误的可能。
...
- 注意：当前配置同时设置了 configRUN\_FREERTOS\_SECURE\_ONLY = 1，表示整个应用（包括调度器）都在安全侧运行，不分支到非安全侧（可能是因为硬件不支持禁用 TrustZone）。
...
- 1 = 启用，支持浮点运算（任务可使用 float/double 类型）。
- 0 = 禁用，不使用 FPU 功能（节省资源，适用于无浮点需求的场景）。
...
- 值：1
- 作用：控制是否启用任务通知功能。
  - 1 = 启用，每个任务可接收通知（轻量级同步机制，可替代信号量/事件组）。
  - 0 = 禁用，不使用任务通知（可节省内存）。
- 优势：相比信号量，任务通知更高效（无需额外的内核对象），适用于一对一的同步场景。
...
- 作用：控制是否启用递归互斥量功能。
  ...
  - 0 = 禁用，不使用递归互斥量。
- 适用场景：任务需在嵌套函数中重复获取同一互斥量时使用（如递归函数访问共享资源）。
...
- 作用：控制是否启用队列集（Queue Sets）功能。
  - 0 = 禁用，不使用队列集。
  - 1 = 启用，可使用 xQueueCreateSet 创建队列集（用于同时等待多个队列/信号量的数据）。
...
#### 5. INCLUDE_xResumeFromISR = 1

- 作用：包含 xResumeFromISR 函数（用于在中断中恢复任务）。
...
#### 13. INCLUDE_xEventGroupSetBitFromISR = 1

- 作用：包含 xEventGroupSetBitFromISR 函数（用于在中断中设置事件组的位）。

#### 14. INCLUDE_xTimerPendFunctionCall = 0

- 作用：不包含 xTimerPendFunctionCall 函数（用于通过定时器守护任务执行回调函数）。
...
#### 17. INCLUDE_xTaskResumeFromISR = 1

- 作用：包含 xTaskResumeFromISR 函数（用于在中断中恢复任务，与 xResumeFromISR 功能相同，可能为兼容性保留）。
...
struct xLIST_ITEM
{
    TickType_t xItemValue;            //排队用的“号码牌”，相当于：这个人排第几，在 延时链表：xItemValue = 唤醒的 Tick，在 就绪链表：优先级（或固定值）
    ListItem_t * pxNext;，            //指向双向链表的前一个成员
    ListItem_t * pxPrevious;          //指向双向链表的后一个成员        O(1) 操作的前提
    void * pvOwner;                    /* “这个人代表谁？”TCB_t * pxTCB，注意注意这个地方，其实是指向的TCB，到底是哪个任务呢？pvOwner = 指向 TCB，调度器从链表里取到                                                 ListItem，立刻能找到对应任务 */
    struct xLIST * pxContainer;        //我在哪个名单里？指向自己当前所在的 List_t，比如就绪链表？阻塞链表？
};
```
...
对应到 List 操作其实只有两种：（后面再讲，这就是我们所说的 RTOS 其实就是链表的操作）
...
#### void vListInitialise（ List_t * const pxList ）：「创建一个空名单」
...
#### void vListInitialiseItem（ ListItem_t * const pxItem ）：让 ListItem 处于“未加入任何链表”的干净状态
...
#### void vListInsertEnd（ List_t * const pxList， ListItem_t * const pxNewListItem ）：「不排序，只插队尾」
...
#### void vListInsert（ List_t * const pxList，ListItem_t * const pxNewListItem ）：「按 xItemValue 排队」
...
#### UBaseType_t uxListRemove（ ListItem_t * const pxItemToRemove ）:O（1） 从任何位置安全删除
...
RTOS 源码其实就是链表的操作，所以在看源码之前一定要先保证自己对于链表的操作比较熟悉了。【C 语言 链表基础知识清晰讲解（黑马）】

> 🌈
>
> 总览
>
> - heap\_1.c：最简单的静态内存分配，只分配不释放，适合资源受限场景。
>
...
##### 1.首先定义了 8 字节对齐！！！内存对齐可以提高 CPU 访问内存的效率，因为大多数 CPU 在访问对齐的内存地址时速度更快
...
> - xTaskResumeAll()：恢复任务调度器，如果有更高优先级的任务在等待，会立即切换
>
...
> - 使用 heapSIZE_MAX - b 可以在不发生实际溢出的情况下进行安全检查
>
> 🌈
>
...
>
> - 要将某个值 x 对齐到 N 字节边界，结果应满足：`结果 = x + （N - 1） & ～（N - 1）`
...
> 为什么需要溢出检查：
>
> - 如果 xWantedSize 已经接近 heapSIZE_MAX，添加对齐字节可能导致溢出
> - 溢出会导致 xWantedSize 变为一个很小的值（无符号整数溢出回绕）
> - 这会造成严重的内存分配错误，甚至系统崩溃

> 示例（溢出情况）：
>
> - 假设 heapSIZE_MAX = 0xFFFFFFFF（32 位系统）
...
##### 插入空闲块函数讲解（一定要熟悉链表的操作！！！）如果刷过链表的力扣，这部分应该看起来很简单！！！
...
> 看代码的时候不要在意：
>
> heapPROTECT\_BLOCK\_POINTER
>
> #if ( configENABLE\_HEAP\_PROTECTOR == 1 )
>
> /\* 启用指针保护模式 \*/
>
> #define heapPROTECT\_BLOCK\_POINTER( pxBlock ) ( ( BlockLink\_t \* ) ( ( ( portPOINTER\_SIZE\_TYPE ) ( pxBlock ) ) ^ xHeapCanary ) )
>
> #else
>
> /\* 禁用指针保护模式（默认） \*/
>
> #define heapPROTECT\_BLOCK\_POINTER( pxBlock ) ( pxBlock )
>
> #endif
>
...
#### vPortDefineHeapRegions 函数-----heap5 的核心初始化函数函数，必须理解
...
##### 地址对齐处理（每一个堆区都要确保对齐地址！！！）这个在 heap4 的时候就有所体会，这个应该比较熟悉了。
...
2. 后续区域处理：检查区域地址顺序：内存区域按地址从低到高的顺序传入！！！
...
### heap5 Free（和 heap4 的逻辑是一样的）因为 Heap_5 本质上就是“支持多块物理内存”的 Heap_4
...
我们只需要关注几个重要的变量就 OK 啦，其他的就自行去源码看看就好啦。

#### 大家可以看到源文件里面使用了好多条件定义，这个根据自己的项目需求就 OK 啦，比如我自己这个就开启了互斥量嘛（在 FreeRTOSConfig.h 中进行配置）
...
> 答案： 不可以。它必须先回到 Ready 状态。调度器只会从 Ready 列表中寻找最高优先级的任务来运行。即使这个任务优先级最高，它也得先去 Ready List 排队，等调度器在下一个 Tick 或触发切换时把它选为 pxCurrentTCB。

#### 这个图一定要记住！

![[assets/feishu-freertos-source-analysis/img-153.jpg]]

### xTaskCreate 函数（以动态创建为例子）
...
#### 申请成功之后先干嘛？申请到 TCB 内存后立刻清零。这是一个好习惯，确保 TCB 里的各种链表指针、计数器在初始化前都是确定的 NULL 或 0，防止野指针。
...
### prvInitialiseNewTask 函数---------任务创建的核心函数（可以简单理解为填充前面创建的 TCB 结构体）
...
- 4. 优先级与互斥量
  ...
  - 传入的任务优先级会被限定在 `configMAX_PRIORITIES` 范围内。例如系统最高优先级为 5，而创建函数传入 10 时，会强制降为 5。
...
### prvAddNewTaskToReadyList 函数----------------将新创建的任务添加到就绪列表中
...
### vTaskDelete 函数---本质就是链表的切换

- 不能"自杀"：正在运行的任务（CPU 正在执行它的代码）无法立即释放自己的栈和 TCB，否则会导致程序跑飞
- 空闲任务"收尸"：自己删除自己时，只是把尸体（TCB）放入"待清理列表"，由空闲任务（Idle Task）在后台真正清理内存
- 他杀立即生效：删除其他不在运行的任务时，可以立即释放资源
...
#### 检查调度器在运行且 任务正在运行（或即将让出）-------------放入终止等待列表
...
自己删自己时，必须立刻"让出" CPU，否则会继续执行已不存在的任务的后续代码。 taskYIELD\_WITHIN\_API(); // 触发上下文切换

### vTaskSuspend 函数---本质就是链表的切换
...
#### 1. 获取任务控制块：如果传入 NULL，表示挂起当前正在运行的任务

#### 2. 从就绪/延迟列表中移除：uxListRemove（ &（ pxTCB->xStateListItem ） ）

- 从就绪列表或延迟列表中移除任务。
- 如果该优先级下没有其他就绪任务，清除就绪位图中的对应位。

#### 3. 处理事件列表项：uxListRemove（ &（ pxTCB->xEventListItem ） ）；

- 如果任务正在等待某个事件（如队列、信号量等），从事件列表中移除。

#### 4. 加入挂起列表：vListInsertEnd（ &xSuspendedTaskList， &（ pxTCB->xStateListItem ） ）；

- 将任务添加到全局挂起列表 xSuspendedTaskList

#### 5. 处理任务通知（如果启用了的话（在 config.h 中定义的哦））
...
#### 如果调度器未开启：这种情况发生在调用 vTaskStartScheduler() 之前挂起任务
...
taskYIELD\_ANY\_CORE\_IF\_USING\_PREEMPTION( pxTCB );

- 宏功能：如果使用抢占式调度，可能触发任务切换

- 特殊处理：恢复的任务不一定立即运行，但确保调度器状态正确

### prvCreateIdleTasks 函数（都以单核为例哈）
...
单核的话就知道pxIdleTaskFunction = prvIdleTask;
...
#### 12.2 循环为每个核心创建空闲任务，其实咱们就按照单核来看就 OK
...
- portPRIVILEGE\_BIT // 优先级为 （tskIDLE\_PRIORITY | portPRIVILEGE\_BIT）// tskIDLE\_PRIORITY = 0，所以是优先级 0// portPRIVILEGE\_BIT 确保任务在特权模式运行

- 使用最小栈大小 configMINIMAL\_STACK\_SIZE

### vTaskStartScheduler 函数-----------FreeRTOS 的核心调度器启动函数 务必掌握！！！
...
#### 3. 用户自定义初始化（可选）这个如果有自己添加的功能可以修改

- 需要在 FreeRTOSConfig.h 中定义 FREERTOS\_TASKS\_C\_ADDITIONS\_INIT
...
xNextTaskUnblockTime = portMAX\_DELAY; // 下一个任务解锁时间

xSchedulerRunning = pdTRUE; // 标记调度器已运行

xTickCount = configINITIAL\_TICK\_COUNT; // 系统节拍计数器
...
#### 5. 启动硬件相关的调度器（ void ） xPortStartScheduler（）

### xPortStartScheduler()函数---硬件相关函数
...
#### 2. 设置 PendSV 和 SysTick 中断优先级 C 语言位运算，这个得熟悉啊

#define portNVIC\_PENDSV\_PRI ( ( ( uint32\_t ) portMIN\_INTERRUPT\_PRIORITY ) << 16UL )

#define portNVIC\_SYSTICK\_PRI ( ( ( uint32\_t ) portMIN\_INTERRUPT\_PRIORITY ) << 24UL )
...
1. 配对使用：
   ...
   - 挂起和恢复的次数必须相等
1. 中断处理：
   - 挂起期间中断会继续执行，但不会触发上下文切换
   - ISR 中可以调用xTaskResumeAll()，但这种情况很少见且需要特别小心
1. 资源保护：
...
### xTaskGetTickCount 函数-------获取当前系统的 Tick 计数
...
- 在 32 位处理器上，虽然单次读取 32 位变量是原子操作，但 FreeRTOS 为了保持跨平台兼容性，仍然保留了临界区保护
1. 与 ISR 的兼容性
- 此函数可以在任务中安全使用
...
- 中断保护机制
  - 函数使用 portTICK\_TYPE\_SET\_INTERRUPT\_MASK\_FROM\_ISR() 和 portTICK\_TYPE\_CLEAR\_INTERRUPT\_MASK\_FROM\_ISR() 实现中断保护
  - 这种机制比标准临界区更轻量，专门针对中断环境优化
  - 仅屏蔽可能修改 xTickCount 的中断（通常是 tick 中断），而非所有中断
- 原子操作保证
...
1. 溢出处理：
   ...
   - 使用 FreeRTOS 提供的时间比较宏： `if(pdTICKS\_GREATER\_THAN(xTickCount, xDelayUntil)) { // 时间已到 }`
1. 低功耗模式：
...
### vTaskSwitchContext 函数-----最核心的任务调度函数之一，负责选择下一个要运行的任务

选择下一个要运行的任务。它实现了基于优先级的抢占式调度策略，支持相同优先级任务的时间片轮转。函数包含了丰富的可选功能，如运行时统计、栈溢出检查、POSIX errno 支持和 TLS 支持，可通过配置宏进行启用或禁用。该函数的高效实现对于 FreeRTOS 的实时性能至关重要，因此许多端口都提供了汇编优化版本。它在系统 tick 中断、任务状态变化和手动请求任务切换时被调用，是 FreeRTOS 调度器的关键组成部分。
...
1. 调用上下文限制：
   - 通常在中断上下文或系统内部调用，应用程序一般不需要直接调用
   - 必须在临界区或中断服务程序中调用，以确保原子性
1. 调度器状态影响：
...
### vTaskPlaceOnEventList 函数--------将当前运行的任务放置到指定的事件列表中并使其进入阻塞状态
...
1. 将任务添加到延迟列表：
   - prvAddCurrentTaskToDelayedList(xTicksToWait, pdTRUE)：将当前任务添加到系统延迟列表，并设置超时时间
   - 第二个参数pdTRUE表示这是一个相对延迟时间
   - 此函数会：
     - 更新任务的阻塞时间
     - 将任务从就绪列表移除
     - 根据超时时间将任务添加到合适的延迟列表
...
### xTaskPriorityInherit 函数----------优先级继承机制
...
### ulTaskGenericNotifyTake 函数----获取任务通知

比队列、信号量等传统同步机制更高效。该函数允许任务等待通知，如果没有立即获得通知，可以选择阻塞指定时间
...
typedef struct QueueDefinition {
    ...
    // 队列/信号量共用联合体
    union {
        QueuePointers_t xQueue;     // 作为队列使用时的数据
        SemaphoreData_t xSemaphore; // 作为信号量使用时的数据
    } u;
    ...
    UBaseType_t uxLength;                   // 队列长度（可容纳的项目数）
    UBaseType_t uxItemSize;                 // 每个项目的大小（字节数）
    
    // 队列锁定状态
    volatile int8_t cRxLock;                // 接收锁定计数
    volatile int8_t cTxLock;                // 发送锁定计数
    
    // 静态/动态内存分配标识
    #if ( ( configSUPPORT_STATIC_ALLOCATION == 1 ) && ( configSUPPORT_DYNAMIC_ALLOCATION == 1 ) )
        uint8_t ucStaticallyAllocated;      // 是否静态分配
    #endif
    
    // 队列集支持
    #if ( configUSE_QUEUE_SETS == 1 )
        struct QueueDefinition * pxQueueSetContainer;  // 所属队列集
    #endif
...
### xQueueGenericCreateStatic 函数------底层实现

- 静态创建各种类型的队列对象（基础队列、信号量、互斥锁等）
- 验证参数有效性并初始化队列结构
- 设置队列属性并准备队列存储区
- 是所有静态队列创建 API（如xQueueCreateStatic、xSemaphoreCreateMutexStatic等）的底层实现
...
#### xQueueGenericReset 函数--------完成队列的全面初始化 了解

- 初始化存储区指针（思想和初始化环形缓冲区的差不多）
- 重置队列状态
- 初始化等待任务列表（ vListInitialise 函数在第一章有讲）
![[assets/feishu-freertos-source-analysis/img-201.jpg]]

### xQueueGenericCreate 函数-----------动态创建队列、信号量或互斥锁
...
### xQueueCreateMutex 函数------实际就是调用的队列创建函数，只不过参数不一样而已！

![[assets/feishu-freertos-source-analysis/img-204.jpg]]

#### prvInitialiseMutex 函数----初始化互斥锁
...
#### 引申：后续的计数信号量、二值信号量都是类似的操作了

![[assets/feishu-freertos-source-analysis/img-206.jpg]]

### xQueueGenericSend 函数--------底层函数
...
#### xQueueGenericSendFromISR 与普通版本 xQueueGenericSend 的关键区别

### xQueueReceive 函数--------队列接收数据
...
#### 阻塞等待逻辑，就是看有没有设置第三个参数 TickType_t xTicksToWait ，如果配置的 0 就立即返回，如果配置了其他就会更新状态等待数据。
...
### prvIsQueueEmpty 和 prvIsQueueFull 就不赘述了，就是判断环形缓冲区空和满
...
/* 定时器本身的定义 */
    ...
    {
        const char * pcTimerName;                                                /**< 文本名称。内核不使用它，仅包含它是为了使调试更容易。 */
        ListItem_t xTimerListItem;                                               /**< 所有内核功能用于事件管理的标准链表项 */
        TickType_t xTimerPeriodInTicks;                                          /**< 定时器到期的速度和频率 */
        void * pvTimerID;                                                            /**< 用于标识定时器的ID。当同一个回调用于多个定时器时，这允许识别定时器 */
        portTIMER_CALLBACK_ATTRIBUTE TimerCallbackFunction_t pxCallbackFunction; /**< 定时器到期时将调用的函数 */
        #if ( configUSE_TRACE_FACILITY == 1 )
            UBaseType_t uxTimerNumber;                                           /**< 由FreeRTOS+Trace等跟踪工具分配的ID */
        #endif
        uint8_t ucStatus;                                                        /**< 包含位，用于指示定时器是静态分配还是动态分配，以及它是否处于活动状态 */
    } xTIMER;

/* The old xTIM
```

### xTimerCreateTimerTask 函数--------创建专门的定时器服务任务

所有定时器的命令处理和到期回调都在这个任务中执行，是整个定时器系统的核心。
...
#### prvCheckForValidListAndQueue 函数-------检查定时器列表和命令队列是否已初始化
...
### xTimerCreate 函数---------创建一个新的软件定时器（static 版本的就不讲了）
...
### xTimerGenericCommandFromTask 函数---------从任务中向定时器服务任务发送通用命令
...
### xTimerGenericCommandFromISR 和 xTimerGenericCommandFromTask 函数一样，只不过内部调用的是 xQueueSendToBackFromISR 函数

### prvProcessReceivedCommands 函数----------处理定时器命令队列中的所有待处理命令
...
##### 定时器操作----无论收到什么命令，先把定时器从活动列表移除

防止命令处理期间定时器到期，确保状态一致。

![[assets/feishu-freertos-source-analysis/img-233.jpg]]

##### 如果先获取时间再读队列，高优先级任务可能抢占，导致时间基准不准

#### 启动/复位等命令的特殊处理

![[assets/feishu-freertos-source-analysis/img-234.jpg]]

Stop 命令只需要清除标志位就 OK，因为我们在一开始就移除了链表。

- 活动定时器：ACTIVE 标志置位 + 在 xActiveTimerList 中
- 停止后：ACTIVE 标志清除 + 不在任何列表中

Change\_Period：新的到期时间 = 当前时间 + 新周期（从"现在"开始），所以不会出现立即到期的情况。

Delete：注意静态分配不能用 vPortFree 就 OK 啦。静态定时器的内存由用户分配，so 不能 vPortFree（&xTimerBuffer）！这不是堆内存。

### 简单的函数就不讲解了

这种设置定时器 ID 获取定时器 ID 啊这种就不赘述了哈
...
### xEventGroupSetBits 函数---------设置事件组中的指定事件位
...
### xEventGroupSync 函数-------所有任务都就绪后再统一行动
...
### xEventGroupClearBits 函数---------清除事件组中的指定事件位
...
### xEventGroupWaitBits 函数-----等待事件组中的某些位被设置
...
##### 恢复调度器并检查上下文切换：这很重要，因为它决定了是否需要手动触发一次任务切换
