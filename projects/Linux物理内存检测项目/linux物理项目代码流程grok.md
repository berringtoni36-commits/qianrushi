...
```python
#!/usr/bin/env python3          # L1: shebang，Linux 下可直接执行
from bpfcc import BPF           # L2: BCC 的 Python 绑定（有的环境是 bcc）
import os                       # L3: 引入了但本文件几乎未用
import time                     # L4: 仅 run() 里 sleep 用
import ctypes                   # L5: 写 delay_map 时用 c_int
```

| 行 | 含义 |
|---:|---|
| 1 | 解释器声明 |
| 2 | **核心依赖**：`BPF` 类完成编译/加载/挂载/map 访问 |
| 3–4 | 标准库；`os` 当前冗余 |
| 5 | 把 Python int 转成 C `int` 写入 BPF array |
...
```python
class ExtFrag:                                                           # L7
    def __init__(self, interval=2, output_extfrag_index=False,            # L8
                 output_unusable_index=False, output_count=False,
                 zone_info=False):
        self.interval = interval                                         # L9
        self.output_extfrag_index = output_extfrag_index                 # L10
        self.output_unusable_index = output_unusable_index               # L11
        self.output_count = output_count                                 # L12
        self.zone_info = zone_info                                       # L13
                                                                         # L14-15 空行
        if self.output_count:                                            # L17
            self.b = BPF(src_file="./bpf/extfraginfo.c")                 # L18
        else:                                                            # L19
            self.b = BPF(src_file="./bpf/fraginfo.c")                    # L20
        delay_key = 0                                                    # L21
        self.b["delay_map"][delay_key] = ctypes.c_int(interval)          # L22
```
...
| 行 | 代码要点 | 深入说明 |
|---:|---|---|
| 7 | `class ExtFrag` | UI 通过这个类拿所有监控数据 |
| 8 | 五个构造参数 | `interval` 采样/刷新秒数；后四个来自 CLI。**真正改变加载行为的主要是 `output_count`** |
| 9–13 | 存成成员 | `output_extfrag_index` / `unusable` / `zone_info` 在本文件**后续几乎不再使用**；列选择在 `exfrag_user.py` 用自己的 `args` 字典 |
| 17–20 | **二选一加载** | `True`→事件 C；`False`→状态 C。**不能同时加载两份** |
| 18/20 | `BPF(src_file=...)` | 同步重活：读文件、编译、`bpf()`、挂探针。成功后 `self.b` 可当 map 字典用 |
| 21–22 | 写 `delay_map[0]` | 用户态配置传入内核。eBPF 里 `delay_map.lookup(&key)` 的 `key=0` 与这里对应 |
...
```python
def calculate_scoreA(self, extfrag_index):          # L25
    extfrag_index_int_part = int(extfrag_index) // 1000   # L26
    extfrag_index_dec_part = int(extfrag_index) % 1000    # L27
    return f"{extfrag_index_int_part:2d}.{extfrag_index_dec_part:03d}"  # L28
```

| 行 | 作用 |
|---:|---|
| 25 | 输入：内核 `score_a`（千分制整数，可能为负） |
| 26 | 整数部分：`-1000//1000 = -1`，`584//1000 = 0` |
| 27 | 小数部分（千分位）：`584 % 1000 = 584` → 显示 `.584` |
| 28 | 格式成宽 2 的整数 + 3 位小数，如 ` 0.584`、`-1.000` |

**注意**：这是展示变换，**不是**重新计算 extfrag 公式。

负数时 Python 的 `%` 行为：`(-1000) % 1000 == 0`，所以 `-1000` → `-1.000`，符合预期。  
若内核返回 `-500` 这类中间负值（本项目 suitable 分支直接 -1000，一般不会），显示要自己心里有数。
...
```python
def get_zone_data(self, filter_node_id=None):     # L35
    zone_data_dict = {}                           # L36  返回容器：{zone名: [各order字典]}
    zone_map = self.b["zone_map"]                 # L37  拿到 BCC 对内核 hash 的代理

    for key, value in zone_map.items():           # L39  遍历所有 (zone_ptr+order) 条目
        comm = value.name.decode('utf-8', 'replace').rstrip('\x00')  # L40
        node_id = value.node_id                   # L41
        if filter_node_id is not None and node_id != filter_node_id:  # L42-43
            continue
        data = {                                  # L44-56 组装一行 UI 所需字段
            'comm': comm,
        ...
        if comm not in zone_data_dict:            # L57-58
            zone_data_dict[comm] = []
        zone_data_dict[comm].append(data)         # L59
        for comm in zone_data_dict:               # L60-61  每次 append 后对所有 zone 重新 sort
...
| 行 | 深入 |
|---:|---|
| 37 | 若加载的是 `extfraginfo.c`，**没有** `zone_map`，这里会异常——所以 `-s` 模式绝不能走这个函数当主路径 |
| 39 | `key` 是 `zone_ptr+order`，本函数**没用 key**，只扫 value |
| 40 | C `char name[32]` → bytes → 去 `\0`；`replace` 避免非法 UTF-8 崩掉 |
| 42–43 | 对应 CLI `-i` |
| 44–56 | 字段一一对应 `struct zone_info`；分数在这里变成字符串 |
| 57–59 | 按 **zone 名字符串**分组，不是按 node；同名跨 node 时可能混在一起（多 NUMA 时要小心） |
| 60–61 | 排序放在内层循环，**每插入一条就全局重排**，正确但 O(n² log n) 味道，数据量小可接受 |
...
```python
def get_view_data(self, filter_node_id=None):     # L64
    zone_data_dict = {}                           # L65
    ret_dict = {}                                 # L66
    ...
    for key, value in zone_map.items():           # L69
        ...
        zone_data_dict[(node_id, comm)] = data    # L82  同一(node,zone)被 order 循环反复覆盖
    sorted_keys = sorted(zone_data_dict.keys())   # L83
...
| 行 | 深入 |
|---:|---|
| 82 | key 是 `(node_id, comm)`，value 不断被更高/不同 order 覆盖，**最终只剩该 zone 某一个 order 的 scoreB** |
| 83–85 | 按 key 排序后拷贝，保证 UI 遍历顺序稳定 |
| 用途 | `-v` 里主要用它**枚举有哪些 (node, zone)**；真正 11 个 order 进度仍靠 `get_zone_data` |
...
```python
def get_nr_zones(self, filter_node_id=None):      # L87
    node_zone_map = {}                            # L88
    ...
    for key, value in zone_map.items():           # L91
        ...
        data = {'scoreB': ..., 'order': ...}      # L98-101  构造了却没用！
        if node_id not in node_zone_map:          # L104-105
            node_zone_map[node_id] = []
        node_zone_map[node_id].append(comm)       # L106  每个 order 都 append 一次同名
    return node_zone_map                          # L108
```

| 行 | 深入 |
|---:|---|
| 98–101 | `data` 算完不用，死代码 |
| 106 | 同一 zone 有 11 个 order → 同一 `comm` 被 append **11 次** |
| 下游 | `get_node_data` 用 `len(list)/11` 反推 zone 数 |
...
def get_node_data(self):                          # L109
    node_data_dict = {}                           # L110
    pgdat_map = self.b["pgdat_map"]               # L111
    zone_data = self.get_nr_zones()               # L112

    for key, value in pgdat_map.items():          # L114
        node_id = value.node_id                   # L115
        nr_zones = int(len(zone_data.get(node_id, []))/11)  # L116 魔法数11
        data = {
            'pgdat_ptr': value.pgdat_ptr,         # L118 实际是 node_start_pfn
            'nr_zones': nr_zones,                 # L119
...
| 行 | 深入 |
|---:|---|
| 111 | 依赖 fraginfo 的 `pgdat_map` |
| 116 | `11 = MAX_ORDER+1 = 0..10`；内核若改 MAX_ORDER 会错 |
| 118 | 字段名 `pgdat_ptr`，内核写入的是 `node_start_pfn`（见 fraginfo L133） |
| 注意 | 没用 C 里已经写好的 `pgdat_info.nr_zones`，反而自己用 len/11 推 |
...
```python
def get_count_data(self):                         # L126
    count_data_list = []                          # L127
    counts_map = self.b["counts_map"]             # L128  仅 extfraginfo 有此 map
    for key, value in counts_map.items():         # L131
        _comm = value.pcomm.decode(...).rstrip('\x00')  # L132
        data = {
            'pcomm': _comm,                       # L134
            'pid': value.pid,                     # L135
            'pfn': value.pfn,                     # L136  最近一次
            'alloc_order': value.alloc_order,     # L137  最近一次
            'fallback_order': value.fallback_order, # L138
            'count': value.count                  # L139  累计
        }
        count_data_list.append(data)              # L143
    count_data_list.sort(key=lambda x: x['count'], reverse=True)  # L146
    return count_data_list                        # L148
```

| 行 | 深入 |
|---:|---|
| 128 | 状态模式加载 fraginfo 时无此 map → 不要在默认模式调 |
| 136–138 | **不是历史**，是最后一次 update 覆盖的快照 |
| 139 | 唯一真正累计的字段 |
| 146 | 热点进程排前面，便于 UI |
...
```c
#include <linux/gfp.h>           // L1: gfp_t 等分配标志相关
#include <linux/mm.h>            // L2: 内存管理通用
#include <linux/mmzone.h>        // L3: zone/pgdat/free_area 等
#include <uapi/linux/ptrace.h>   // L4: pt_regs，kprobe 上下文

#define MAX_ORDER 10             // L6: 与扫描阶数 0..10 一致（共11档）
```

| 行 | 深入 |
|---:|---|
| 1–4 | BCC 会结合内核头/类型生成可编译的 BPF 程序；版本不对可能编不过 |
| 6 | 现代内核 MAX_ORDER 可能不同；这里写死 10 |
...
struct alloc_context {
  ...
  struct zoneref *preferred_zoneref;      // L31: 本程序实际用到的关键字段
  int migratetype;                        // L32
...
| 行 | 深入 |
|---:|---|
| 整段 | **不是**稳定 UAPI，是为了让 BCC/编译器知道参数布局，从而在 kprobe 里写 `ac->preferred_zoneref...` |
| 风险 | 内核结构字段顺序/对齐变化 → 读到野指针级错误 |
| 实际使用 | 主要用到 `preferred_zoneref`（L113） |
...
```c
static int unusable_free_index(unsigned int order,           // L48
                               struct contig_page_info *info) {
  if (info->free_pages == 0)                                 // L50
    return 1000;                                             // L51: 100% 不可用
  return div_u64(                                            // L52: BPF 安全的 64 位除法
      (info->free_pages - (info->free_blocks_suitable << order)) * 1000ULL, // L53
      info->free_pages);                                     // L54
}
```
...
\[
score\_b = \frac{free\_pages - suitable\_pages}{free\_pages}\times 1000
\]
...
| 行 | 深入 |
|---:|---|
| 50–51 | 没有空闲页：直接最差 1000 |
| 52 | 不用普通 `/`，避免生成 BPF 不支持的运行时 |
| 53 | 分子：空闲但“组不成该 order 连续块”的页数 ×1000 |
| 54 | 除以总空闲页 → 比例 |
...
static int __fragmentation_index(unsigned int order,
                                 struct contig_page_info *info) {
  unsigned long requested = 1UL << order;        // L59: 请求页数
  if (WARN_ON_ONCE(order > MAX_ORDER))           // L60
    return 0;                                    // L61
  if (!info->free_blocks_total)                  // L62
    return 0;                                    // L63: 无任何空闲块
  if (info->free_blocks_suitable)                // L64
    return -1000;                                // L65: 有合适块 → “可分配”语义
  return 1000 -                                  // L66-68
...
| 行 | 深入 |
|---:|---|
| 59 | order=3 → requested=8 页 |
| 62–63 | 完全没空闲块，返回 0 |
| 64–65 | **快速路径**：只要 suitable>0，指数直接 -1000（显示 -1.000） |
| 66–68 | 无 suitable 时：值越高越像“外部碎片”，越低越像“总量不足” |

这与内核 fragmentation index 思想一致，刻度×1000 方便整数运算。
...
static void fill_contig_page_info(struct zone *zone,
                                  ...
                                  struct contig_page_info *info) {
  unsigned int order;                            // L74
  info->free_pages = 0;                          // L75
  info->free_blocks_total = 0;                   // L76
  info->free_blocks_suitable = 0;                // L77
  for (order = 0; order <= MAX_ORDER; order++) { // L78 扫全部阶
    unsigned long blocks;                        // L79
    unsigned long nr_free;                       // L80
    bpf_probe_read_kernel(&nr_free, sizeof(nr_free),          // L81
                          &zone->free_area[order].nr_free);   // L82
    blocks = nr_free;                            // L83
    info->free_blocks_total += blocks;           // L84  块个数累加
    info->free_pages += blocks << order;         // L85  块→页
    if (order >= suitable_order)                 // L86
      info->free_blocks_suitable += blocks << (order - suitable_order); // L87
  }
}
```
...
| 行 | 深入 |
|---:|---|
| 81–82 | **必须**用 helper 读内核内存；直接 `zone->...` 在严格 BPF 里常被拒绝 |
| 84 | 不论块大小，1 个 order0 块和 1 个 order5 块都 +1 |
| 85 | order3 有 2 块 → +`2<<3`=+16 页 |
| 86–87 | 只把 **≥ 目标阶** 的块算 suitable，并折算：  
  目标 suitable_order=2 时，1 个 order4 块 → `1<<(4-2)=4` 个“虚拟 order2 块” |

**为什么要折算？**  
高阶大块可以拆成多个低阶请求；统计“能满足多少次 order=k 的分配”。
...
```c
  ...
  if (last_time && (current_time - *last_time < delay * 1000000000)) { // L103
    return 0;                                    // L104 未到间隔则跳过
  }
```
...
|---:|---|---|
| 95 | 取 now | OK |
| 96 | 查“上次采样时间” | 用 **now 当 key** 去查，几乎永远 miss |
| 98–101 | 读间隔秒 | OK；但 delay_ptr 为空时 delay 垃圾值 |
| 103–104 | now-last < delay秒 则 return | 因 96 常 miss，条件常不成立 → **几乎不节流** |
| 166 | update 也用 current_time 当 key | 存了一堆不同 key，帮不上下次 lookup |
...
```c
  for (i = 0; i < MAX_NR_ZONES; i++) {           // L115
    ...
    zref = &pgdat->node_zonelists[ZONELIST_FALLBACK]._zonerefs[i]; // L121
    z = zref->zone;                              // L122
    if (!z)                                      // L123
      continue;                                  // L124
```
...
| 行 | 深入 |
|---:|---|
| 115 | 最多扫 `MAX_NR_ZONES` 个槽 |
| 121 | 使用 **FALLBACK** zonelist（允许回退的分配列表） |
| 122–124 | 空槽跳过 |
...
```c
    for (a_order = 0; a_order <= MAX_ORDER; ++a_order) {  // L147
      ...
      zone_key = zone_data.zone_ptr + zone_data.order;    // L149

      struct contig_page_info ctg_info;                   // L151
      fill_contig_page_info(z, a_order, &ctg_info);       // L152
      ...
      zone_data.score_b = tmp;                            // L158
      index = __fragmentation_index(a_order, &ctg_info);  // L159
      zone_data.score_a = index;                          // L160

      zone_map.update(&zone_key, &zone_data);             // L162
      zone_key++;                                         // L163 无效：下轮重算 key
    }
```

| 行 | 深入 |
|---:|---|
| 149 | key 唯一性依赖 zone 对象地址间隔 >> 11 |
| 152 | 每次 order 都完整扫 0..10 freelist（有重复计算，逻辑简单） |
| 157–160 | 先 B 后 A，写入同一 zone_data |
| 162 | 覆盖写最新快照 |
| 163 | `zone_key++` 无实际作用 |
...
```c
TRACEPOINT_PROBE(kmem, mm_page_alloc_extfrag) {  // L20
  u64 *last_time, current_time;                  // L21
  current_time = bpf_ktime_get_ns();             // L22
  last_time = last_time_map.lookup(&current_time); // L23 ⚠️ 同 bug
  int key = 0;                                   // L24
  int *delay_ptr = delay_map.lookup(&key);       // L25
  int delay;                                     // L26 ⚠️ 可能未初始化
  if (delay_ptr) {                               // L27
    delay = *delay_ptr;                          // L28
  }
  if (last_time && (current_time - *last_time < delay * 1000000000)) { // L30
    return 0;                                    // L31
  }

  struct data_t *data, zero = {};                // L34 zero 全 0
  pid_t pid = bpf_get_current_pid_tgid() >> 32;  // L35 高32位=TGID/PID

  data = counts_map.lookup(&pid);                // L38
  if (!data) {                                   // L39 首次该 pid
    zero.pid = pid;                              // L41
    zero.pfn = args->pfn;                        // L42 TP 参数
    zero.alloc_order = args->alloc_order;        // L43
    zero.fallback_order = args->fallback_order;  // L44
    zero.count = 1;                              // L45
    bpf_get_current_comm(&zero.pcomm, sizeof(zero.pcomm)); // L46
    counts_map.update(&pid, &zero);              // L47
  } else {                                       // L48 已存在
    data->count += 1;                            // L50 累计
    data->pfn = args->pfn;                       // L51 覆盖最近
...
| 行 | 深入 |
|---:|---|
| 20 | BCC 宏：生成挂载 `kmem:mm_page_alloc_extfrag` 的处理函数；`args` 为事件参数结构 |
| 35 | `bpf_get_current_pid_tgid()` = `(u64)pid<<32 | tid` |
| 42–44 | 来自内核 tracepoint 字段，表示“这次 fallback 相关的阶与页” |
| 46/54 | 读当前任务 comm（进程名） |
| 50 vs 51–53 | **count 累加**；其余字段**只保留最近一次** |
| 缺失 | 本文件**没有**在成功采集后 `last_time_map.update`！  
  对比 fraginfo L166 有 update。  
  即便修了 lookup key，extfraginfo 也少了“记录上次时间”的一步 → 节流更不完整 |

> 再强调：`counts_map` 输出应读成  
> **“某 PID 的 fallback 次数 + 最近一次事件快照”**，不是完整日志。
...
|---:|---|
| 44 | 很多笔记本默认终端不够 → 卡在这 |
| 45 | `_boo=True` 但函数**没有 return _boo**，外面的 `_boo` 基本一直是 False（L69 初始化）→ `if not _boo` 几乎恒真 |
| 含义 | 尺寸检查更多是“阻塞直到够大”，而不是用 `_boo` 门控 |
...
| 细节 | 说明 |
|---|---|
| 不 `sleep` | 循环末尾 view 只 refresh，靠时间戳节流重绘 |
| 每次 `_show` 重建 bars | 可能闪烁/开销大 |
| scoreB 解析 | `strip().split()[0]` 防格式异常 |
