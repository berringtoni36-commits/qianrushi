...
/*-----------------------------------------------------------*/
	...
	static void prvInitialiseMutex( Queue_t *pxNewQueue )
	{
		if( pxNewQueue != NULL )
		{
			...
			pxNewQueue->pxMutexHolder = NULL;/*当队列用于互斥信号量时，将其指向NULL表示没有任务持有该信号量*/
			pxNewQueue->uxQueueType = queueQUEUE_IS_MUTEX;/*设置队列类型为互斥量*/

			/* In case this is a recursive mutex. */
			pxNewQueue->u.uxRecursiveCallCount = 0;/*初始化递归调用计数为0*/

			traceCREATE_MUTEX( pxNewQueue );/*创建互斥量时进行跟踪*/

			/* Start with the semaphore in the expected state. */
			/*当互斥信号量创建成功后释放一次资源，使其初始状态为可用，未被占用*/
...
/*-----------------------------------------------------------*/
	...
	QueueHandle_t xQueueCreateMutex( const uint8_t ucQueueType )/*创建互斥量函数*/
	{
	Queue_t *pxNewQueue;
		...
		taskENTER_CRITICAL();
		{
			...
			if( uxMessagesWaiting > ( UBaseType_t ) 0 )
			{
				...
				if( xJustPeeking == pdFALSE )
				{
					...
					pxQueue->uxMessagesWaiting = uxMessagesWaiting - 1;/*执行消息量减一操作*/
			...
			if( prvIsQueueEmpty( pxQueue ) != pdFALSE )
			{
				...
				#if ( configUSE_MUTEXES == 1 )
				{
					if( pxQueue->uxQueueType == queueQUEUE_IS_MUTEX )
					{
						taskENTER_CRITICAL();
						{
							/*只有在高优先级任务由于等待互斥量被阻塞时才去提升获取互斥量的低优先级任务的优先级*/
	...
	if( pxQueue->uxItemSize == ( UBaseType_t ) 0 )
	{
		#if ( configUSE_MUTEXES == 1 )
		{
			if( pxQueue->uxQueueType == queueQUEUE_IS_MUTEX )
			{
				...
				/*当释放互斥量时，如果该任务由于优先级继承被提升了优先级，该函数用于恢复其原始优先级*/
