...
- 第2阶段：裸机驱动逐个掌握
- 第3阶段：PID 闭环调速
- 第4阶段：FreeRTOS移植 + 应用层任务框架
- 第5阶段：Bootloader/IAP 固件升级
- 第6阶段：调试能力专项训练
- 第7阶段：综合复现
- 第8阶段：高频面试问答
...
```mermaid
%%{init: {"theme":"base","flowchart":{"htmlLabels":true,"curve":"basis","useMaxWidth":true,"nodeSpacing":36,"rankSpacing":46},"themeVariables":{"fontFamily":"Microsoft YaHei, PingFang SC, Noto Sans CJK SC, Arial, sans-serif","fontSize":"14px","primaryColor":"#ffffff","primaryTextColor":"#111827","primaryBorderColor":"#334155","lineColor":"#475569","tertiaryColor":"#f8fafc","clusterBkg":"#ffffff","clusterBorder":"#cbd5e1"}}}%%
flowchart TD
    M(("main()"))
    H["Hardware_<br/>Init()<br/>硬件初始化<br/>NVIC / delay<br/>TIM1 PWM<br/>TIM2 编码器 / 传感器<br/>LCD / UART / DMA"]
    S["System_<br/>Init()<br/>系统状态初始化<br/>g_system<br/>State 初值"]
    C["StartTask_<br/>Create()<br/>创建开始任务<br/>让 RTOS 接管启动"]
    V["vTaskStart<br/>Scheduler()<br/>启动 FreeRTOS 调度器"]
    T["Start<br/>Task<br/>创建业务任务<br/>再初始化 TIM4<br/>避免中断早到"]
    R(("多任务运行"))

    M --> H --> S --> C --> V --> T --> R

    classDef entry fill:#e0f2fe,stroke:#0369a1,stroke-width:2px,color:#0f172a;
    classDef step fill:#ffffff,stroke:#475569,stroke-width:1.4px,color:#0f172a;
    classDef key fill:#ecfdf5,stroke:#047857,stroke-width:2px,color:#064e3b;
    classDef run fill:#fef9c3,stroke:#a16207,stroke-width:2px,color:#422006;
    class M entry;
    class H,S,C,V step;
    class T key;
    class R run;
...
flowchart TD
    subgraph RTOS["FreeRTOS 调度层"]
        R1["任务调度<br/>优先级 / 时间片<br/>阻塞唤醒"]
        R2["同步通信<br/>互斥量 / 信号量<br/>队列"]
        R3["中断衔接<br/>SysTick<br/>PendSV / 外设中断"]
    end

    subgraph APP["APP_TASK 应用层"]
        A1["模式管理<br/>待机 / 手动<br/>自动 / 防回流"]
        A2["业务算法<br/>风速融合<br/>PID 目标 / 事件判断"]
        A3["状态中枢<br/>g_system<br/>State<br/>统一共享数据"]
    end

    subgraph BSP["BSP 驱动层"]
        B1["执行器<br/>MOTOR / BEEP / LCD"]
        B2["传感器<br/>DHT11 / MQ2<br/>编码器 / 按键"]
        B3["通信与存储<br/>UART + DMA<br/>SPI / IAP / CRC32"]
    end

    RTOS -->|"调度任务<br/>提供同步机制"| APP
    APP -->|"调用驱动接口<br/>不直接碰寄存器"| BSP
    BSP -->|"采样值<br/>执行结果"| APP

    classDef rtos fill:#eff6ff,stroke:#2563eb,stroke-width:1.6px,color:#0f172a;
    classDef app fill:#ecfdf5,stroke:#059669,stroke-width:1.6px,color:#0f172a;
    classDef bsp fill:#fff7ed,stroke:#ea580c,stroke-width:1.6px,color:#0f172a;
    class R1,R2,R3 rtos;
    class A1,A2,A3 app;
    class B1,B2,B3 bsp;
```
...
%%{init: {"theme":"base","flowchart":{"htmlLabels":true,"curve":"basis","useMaxWidth":true,"nodeSpacing":36,"rankSpacing":46},"themeVariables":{"fontFamily":"Microsoft YaHei, PingFang SC, Noto Sans CJK SC, Arial, sans-serif","fontSize":"14px","primaryColor":"#ffffff","primaryTextColor":"#111827","primaryBorderColor":"#334155","lineColor":"#475569","tertiaryColor":"#f8fafc","clusterBkg":"#ffffff","clusterBorder":"#cbd5e1"}}}%%
flowchart TD
    DHT["DHT11<br/>temperature<br/>humidity"] --> SENSOR["Sensor<br/>Task<br/>周期采集传感器"]
    MQ2["MQ2<br/>gas<br/>Concentration"] --> SENSOR
    ENC["TIM2 编码器<br/>pulse count"] --> SPEED["SpeedCalc<br/>Task<br/>TIM4 唤醒测速"]
    KEY["KEY<br/>mode<br/>level event"] --> STATE(("g_system<br/>State<br/>互斥量保护<br/>系统状态中枢"))

    SENSOR -->|"写 temp<br/>humi / gas"| STATE
    SPEED -->|"写 actualRPM"| STATE
    STATE -->|"读环境量"| WIND["WindSpeed<br/>Task<br/>融合计算风速"]
    WIND -->|"写 windSpeed<br/>PWM<br/>写 target<br/>RPM"| STATE
    STATE -->|"读目标与实测"| MOTOR["MotorControl<br/>Task<br/>PID 输出 PWM"]
    MOTOR --> PWM["TIM1 PWM<br/>CCR<br/>CH1 / CH1N"]
    PWM --> FAN["有刷直流电机"]
    STATE --> UI["UIDisplay<br/>Task<br/>LCD 刷新"]
    UI --> LCD["LCD 状态显示"]

    classDef input fill:#eff6ff,stroke:#2563eb,stroke-width:1.4px,color:#0f172a;
    classDef task fill:#ffffff,stroke:#475569,stroke-width:1.4px,color:#0f172a;
    classDef state fill:#ecfdf5,stroke:#047857,stroke-width:2.6px,color:#064e3b;
    classDef output fill:#fff7ed,stroke:#ea580c,stroke-width:1.6px,color:#0f172a;
    class DHT,MQ2,ENC,KEY input;
    class SENSOR,SPEED,WIND,MOTOR,UI task;
    class STATE state;
    class PWM,FAN,LCD output;
```
...
%%{init: {"theme":"base","flowchart":{"htmlLabels":true,"curve":"basis","useMaxWidth":true,"nodeSpacing":36,"rankSpacing":46},"themeVariables":{"fontFamily":"Microsoft YaHei, PingFang SC, Noto Sans CJK SC, Arial, sans-serif","fontSize":"14px","primaryColor":"#ffffff","primaryTextColor":"#111827","primaryBorderColor":"#334155","lineColor":"#475569","tertiaryColor":"#f8fafc","clusterBkg":"#ffffff","clusterBorder":"#cbd5e1"}}}%%
flowchart TD
    CLK["开 GPIO 时钟<br/>RCC_APB2<br/>Periph<br/>ClockCmd"]
    MODE["配置引脚模式<br/>Out_PP 输出<br/>IPU 上拉输入"]
    RW["读写电平<br/>SetBits<br/>ResetBits<br/>ReadInput"]
    BEEP["蜂鸣器<br/>PB15 输出推挽"]
    KEY["按键<br/>上拉输入<br/>按下为低电平"]
    ...
    IDLE["IDLE<br/>空闲等待"]
    DEB["DEBOUNCE<br/>消抖 30ms"]
    PRESSED["PRESSED<br/>等待松手<br/>或长按"]
    LONG["LONG_PRESS<br/>长按事件<br/>已触发"]
    SHORT["SHORT_PRESS<br/>短按事件"]

    KEY -->|"检测到低电平"| IDLE
    IDLE -->|"按下"| DEB
    DEB -->|"30ms 后仍按下"| PRESSED
    DEB -->|"30ms 内松开"| IDLE
    PRESSED -->|"超过 1000ms"| LONG
    PRESSED -->|"松手且未长按"| SHORT
    SHORT --> IDLE
    LONG -->|"松手"| IDLE

    classDef gpio fill:#eff6ff,stroke:#2563eb,stroke-width:1.4px,color:#0f172a;
    classDef dev fill:#fff7ed,stroke:#ea580c,stroke-width:1.4px,color:#0f172a;
    classDef state fill:#ffffff,stroke:#475569,stroke-width:1.4px,color:#0f172a;
    classDef event fill:#ecfdf5,stroke:#047857,stroke-width:1.8px,color:#064e3b;
    class CLK,MODE,RW gpio;
    class BEEP,KEY dev;
    class IDLE,DEB,PRESSED,LONG state;
    class SHORT event;
```
...
%%{init: {"theme":"base","flowchart":{"htmlLabels":true,"curve":"basis","useMaxWidth":true,"nodeSpacing":36,"rankSpacing":46},"themeVariables":{"fontFamily":"Microsoft YaHei, PingFang SC, Noto Sans CJK SC, Arial, sans-serif","fontSize":"14px","primaryColor":"#ffffff","primaryTextColor":"#111827","primaryBorderColor":"#334155","lineColor":"#475569","tertiaryColor":"#f8fafc","clusterBkg":"#ffffff","clusterBorder":"#cbd5e1"}}}%%
flowchart TD
    MCU1["MCU 输出模式<br/>拉低约20ms<br/>拉高约30us"]
    DHT["DHT11 应答<br/>80us 低<br/>80us 高"]
    MCU2["MCU 输入模式<br/>逐位采样 40bit"]
    BUF["BUF[0..4]<br/>湿度 / 温度<br/>校验和"]
    STATE["g_system<br/>State<br/>temperature<br/>humidity"]
    WIND["WindSpeed<br/>Task<br/>参与风速融合"]

    MCU1 --> DHT --> MCU2 --> BUF
    BUF -->|"校验通过"| STATE --> WIND
    BUF -.->|"校验失败<br/>丢弃"| MCU1

    classDef host fill:#eff6ff,stroke:#2563eb,stroke-width:1.5px,color:#0f172a;
    classDef sensor fill:#fff7ed,stroke:#ea580c,stroke-width:1.5px,color:#0f172a;
    classDef data fill:#ecfdf5,stroke:#047857,stroke-width:1.8px,color:#064e3b;
    class MCU1,MCU2 host;
    class DHT sensor;
    class BUF,STATE,WIND data;
```
...
![[projects/RTOS项目/源码/dht11-timing.svg]]

**详细步骤**：
| 步骤 | DATA电平变化 | 谁控制总线 | 作用 |
|---|---|---|---|
| 起始 | 低约20ms → 高约30us | MCU | 告诉DHT11开始通信 |
| 应答 | 低约80us → 高约80us | DHT11 | 表示传感器已响应 |
| 发送位 | 低50us → 高电平一段时间 | DHT11 | 高电平宽度决定0/1 |
| 判位 | 延时约30us后读DATA | MCU | 读到低=0，读到高=1 |
| 校验 | 5字节求和 | MCU | 校验失败则丢弃 |
...
flowchart TD
    subgraph ZERO["数据 0"]
        ZL["DATA 低<br/>50us"]
        ZH["DATA 高<br/>26~28us"]
        ZS["约30us后采样<br/>已经回低<br/>读 0"]
        ZL --> ZH --> ZS
    end
    ...
    subgraph ONE["数据 1"]
        OL["DATA 低<br/>50us"]
        OH["DATA 高<br/>约70us"]
        OS["约30us后采样<br/>仍然为高<br/>读 1"]
        OL --> OH --> OS
    end

    ZERO --- ONE

    classDef low fill:#eff6ff,stroke:#2563eb,stroke-width:1.6px,color:#0f172a;
    classDef high fill:#ecfdf5,stroke:#059669,stroke-width:1.6px,color:#064e3b;
    classDef sample fill:#fff7ed,stroke:#ea580c,stroke-width:1.6px,color:#0f172a;
    class ZL,OL low;
    class ZH,OH high;
    class ZS,OS sample;
```
...
%%{init: {"theme":"base","flowchart":{"htmlLabels":true,"curve":"basis","useMaxWidth":true,"nodeSpacing":36,"rankSpacing":46},"themeVariables":{"fontFamily":"Microsoft YaHei, PingFang SC, Noto Sans CJK SC, Arial, sans-serif","fontSize":"14px","primaryColor":"#ffffff","primaryTextColor":"#111827","primaryBorderColor":"#334155","lineColor":"#475569","tertiaryColor":"#f8fafc","clusterBkg":"#ffffff","clusterBorder":"#cbd5e1"}}}%%
flowchart TD
    MQ2["MQ2 传感器<br/>AO 模拟输出"]
    PA4["PA4<br/>ADC1<br/>Channel 4"]
    ADC["ADC1<br/>12bit: 0~4095"]
    READ["MQ2_Get<br/>AdcValue()<br/>等待 EOC 后读值"]
    AVG["10 次采样平均<br/>降低瞬时噪声"]
    GAS["MQ2_Get<br/>Gas<br/>Concen<br/>tration()<br/>ADC -> 电压 -> ppm"]
    MUTEX["g_dataMutex<br/>保护共享状态"]
    STATE["g_system<br/>State<br/>gas<br/>Concentration"]
    WIND["WindSpeed<br/>Task<br/>风速融合"]
    ANTI["AntiBackflow<br/>Task<br/>防回流判断"]
    LCD["UIDisplay<br/>Task<br/>LCD 显示"]

    MQ2 -->|"模拟电压"| PA4 --> ADC --> READ --> AVG --> GAS
    GAS -->|"gasValue"| MUTEX --> STATE
    STATE --> WIND
    STATE --> ANTI
    STATE --> LCD

    classDef sense fill:#eff6ff,stroke:#2563eb,stroke-width:1.5px,color:#0f172a;
    classDef process fill:#ecfdf5,stroke:#059669,stroke-width:1.5px,color:#0f172a;
    classDef system fill:#fff7ed,stroke:#ea580c,stroke-width:1.5px,color:#0f172a;
    classDef center fill:#fef9c3,stroke:#a16207,stroke-width:2px,color:#422006;
    class MQ2,PA4,ADC sense;
    class READ,AVG,GAS process;
    class MUTEX,STATE center;
    class WIND,ANTI,LCD system;
```
...
flowchart TD
    V["模拟电压<br/>0V ~ 3.3V"]
    Q["ADC 量化<br/>12bit<br/>4096 格"]
    D["数字值<br/>0 ~ 4095"]
    P["软件换算<br/>Voltage<br/>RS / ppm"]
    G["气体浓度<br/>gas<br/>Concentration"]

    V --> Q --> D --> P --> G

    classDef analog fill:#eff6ff,stroke:#2563eb,stroke-width:1.6px,color:#0f172a;
    classDef compute fill:#ecfdf5,stroke:#059669,stroke-width:1.6px,color:#0f172a;
    classDef result fill:#fef9c3,stroke:#a16207,stroke-width:2px,color:#422006;
    class V analog;
    class Q,D,P compute;
    class G result;
```
...
%%{init: {"theme":"base","flowchart":{"htmlLabels":true,"curve":"basis","useMaxWidth":true,"nodeSpacing":36,"rankSpacing":46},"themeVariables":{"fontFamily":"Microsoft YaHei, PingFang SC, Noto Sans CJK SC, Arial, sans-serif","fontSize":"14px","primaryColor":"#ffffff","primaryTextColor":"#111827","primaryBorderColor":"#334155","lineColor":"#475569","tertiaryColor":"#f8fafc","clusterBkg":"#ffffff","clusterBorder":"#cbd5e1"}}}%%
flowchart TD
    S1["开时钟<br/>GPIOA + ADC1"]
    S2["配 ADC 时钟<br/>72MHz / 6<br/>= 12MHz"]
    S3["PA4 模拟输入<br/>GPIO_Mode_AIN"]
    S4["ADC 工作模式<br/>独立 / 单通道<br/>单次 / 软件触发<br/>右对齐"]
    S5["使能并校准<br/>上电初始化时做一次"]
...
%%{init: {"theme":"base","flowchart":{"htmlLabels":true,"curve":"basis","useMaxWidth":true,"nodeSpacing":36,"rankSpacing":46},"themeVariables":{"fontFamily":"Microsoft YaHei, PingFang SC, Noto Sans CJK SC, Arial, sans-serif","fontSize":"14px","primaryColor":"#ffffff","primaryTextColor":"#111827","primaryBorderColor":"#334155","lineColor":"#475569","tertiaryColor":"#f8fafc","clusterBkg":"#ffffff","clusterBorder":"#cbd5e1"}}}%%
flowchart TD
    MQ2["当前章节<br/>MQ2 + ADC 采集"]
    LCD["下一章<br/>LCD + SPI 显示"]
    DEEP["深入方向<br/>滑动平均<br/>多通道扫描<br/>DMA"]
    SYS["系统联动<br/>WindSpeed<br/>Task<br/>风速融合"]

    MQ2 --> LCD
    MQ2 --> DEEP
    MQ2 --> SYS

    classDef current fill:#fef9c3,stroke:#a16207,stroke-width:2px,color:#422006;
    classDef next fill:#eff6ff,stroke:#2563eb,stroke-width:1.5px,color:#0f172a;
    classDef deep fill:#ecfdf5,stroke:#059669,stroke-width:1.5px,color:#0f172a;
    class MQ2 current;
    class LCD next;
    class DEEP,SYS deep;
```
...
%%{init: {"theme":"base","flowchart":{"htmlLabels":true,"curve":"basis","useMaxWidth":true,"nodeSpacing":36,"rankSpacing":46},"themeVariables":{"fontFamily":"Microsoft YaHei, PingFang SC, Noto Sans CJK SC, Arial, sans-serif","fontSize":"14px","primaryColor":"#ffffff","primaryTextColor":"#111827","primaryBorderColor":"#334155","lineColor":"#475569","tertiaryColor":"#f8fafc","clusterBkg":"#ffffff","clusterBorder":"#cbd5e1"}}}%%
flowchart TD
    UI["UIDisplay<br/>Task<br/>200ms 刷新"]
    FORMAT["sprintf<br/>拼接显示字符串"]
    GUI["Show_Str<br/>GUI 文本接口"]
    LCDIO["LCD_WR_REG<br/>LCD_WR_DATA<br/>DC 区分命令和数据"]
    SPI["SPI_WriteByte<br/>MOSI + SCK<br/>逐位发送"]
    PANEL["ST7735S LCD<br/>128 x 160 TFT"]

    UI --> FORMAT --> GUI --> LCDIO --> SPI --> PANEL

    classDef task fill:#eff6ff,stroke:#2563eb,stroke-width:1.5px,color:#0f172a;
    classDef api fill:#ecfdf5,stroke:#059669,stroke-width:1.5px,color:#0f172a;
    classDef bus fill:#fff7ed,stroke:#ea580c,stroke-width:1.5px,color:#0f172a;
    classDef out fill:#fef9c3,stroke:#a16207,stroke-width:2px,color:#422006;
    class UI task;
    class FORMAT,GUI,LCDIO api;
    class SPI bus;
    class PANEL out;
```
...
flowchart TD
    ...
    classDef power fill:#fef2f2,stroke:#dc2626,stroke-width:1.8px,color:#7f1d1d;
    classDef mos fill:#eff6ff,stroke:#2563eb,stroke-width:1.5px,color:#0f172a;
    classDef motor fill:#fef9c3,stroke:#a16207,stroke-width:2.2px,color:#422006;
    classDef mode fill:#ecfdf5,stroke:#059669,stroke-width:1.5px,color:#064e3b;
    classDef danger fill:#fee2e2,stroke:#b91c1c,stroke-width:2px,color:#7f1d1d;
    class V,G power;
    class Q1,Q2,Q3,Q4 mos;
    class M motor;
    class FWD,REV mode;
    class BAD danger;
```
...
%%{init: {"theme":"base","flowchart":{"htmlLabels":true,"curve":"basis","useMaxWidth":true,"nodeSpacing":36,"rankSpacing":46},"themeVariables":{"fontFamily":"Microsoft YaHei, PingFang SC, Noto Sans CJK SC, Arial, sans-serif","fontSize":"14px","primaryColor":"#ffffff","primaryTextColor":"#111827","primaryBorderColor":"#334155","lineColor":"#475569","tertiaryColor":"#f8fafc","clusterBkg":"#ffffff","clusterBorder":"#cbd5e1"}}}%%
flowchart TD
    TIM["TIM1 计数器<br/>72MHz<br/>(PSC+1)"]
    CNT["CNT 从 0<br/>计到 ARR<br/>ARR = 999"]
    CMP{"CNT < CCR ?<br/>CCR = 500"}
    HIGH["输出高电平<br/>持续 500 个计数"]
    LOW["输出低电平<br/>持续 500 个计数"]
    DUTY["占空比 50%<br/>CCR / (ARR+1)"]
    MOTOR["平均电压变大<br/>电机转速升高"]

    TIM --> CNT --> CMP
    CMP -->|"是"| HIGH --> DUTY
    CMP -->|"否"| LOW --> DUTY
    DUTY --> MOTOR
    CNT -.->|"到 ARR 后<br/>清零重来"| CNT

    classDef timer fill:#eff6ff,stroke:#2563eb,stroke-width:1.5px,color:#0f172a;
    classDef judge fill:#fef9c3,stroke:#a16207,stroke-width:2px,color:#422006;
    classDef signal fill:#ecfdf5,stroke:#059669,stroke-width:1.5px,color:#064e3b;
    classDef out fill:#fff7ed,stroke:#ea580c,stroke-width:1.8px,color:#0f172a;
    class TIM,CNT timer;
    class CMP judge;
    class HIGH,LOW,DUTY signal;
    class MOTOR out;
```
...
%%{init: {"theme":"base","flowchart":{"htmlLabels":true,"curve":"basis","useMaxWidth":true,"nodeSpacing":36,"rankSpacing":46},"themeVariables":{"fontFamily":"Microsoft YaHei, PingFang SC, Noto Sans CJK SC, Arial, sans-serif","fontSize":"14px","primaryColor":"#ffffff","primaryTextColor":"#111827","primaryBorderColor":"#334155","lineColor":"#475569","tertiaryColor":"#f8fafc","clusterBkg":"#ffffff","clusterBorder":"#cbd5e1"}}}%%
flowchart TD
    OC["TIM1 输出比较<br/>PWM 模式 1"]
    CH1["CH1 / PA8<br/>主输出"]
    CH1N["CH1N / PB13<br/>互补输出"]
    DT["BDTR 死区插入<br/>DTG=100<br/>约 5.6us"]
    SAFE["切换瞬间<br/>两路都关断<br/>避免上下管直通"]
    HBRIDGE["H 桥输入<br/>驱动电机方向<br/>和速度"]

    OC --> CH1 --> DT --> SAFE --> HBRIDGE
    OC --> CH1N --> DT

    classDef timer fill:#eff6ff,stroke:#2563eb,stroke-width:1.5px,color:#0f172a;
    classDef signal fill:#ecfdf5,stroke:#059669,stroke-width:1.5px,color:#064e3b;
    classDef safety fill:#fef9c3,stroke:#a16207,stroke-width:2px,color:#422006;
    classDef output fill:#fff7ed,stroke:#ea580c,stroke-width:1.8px,color:#0f172a;
    class OC timer;
    class CH1,CH1N signal;
    class DT,SAFE safety;
    class HBRIDGE output;
```
...
%%{init: {"theme":"base","flowchart":{"htmlLabels":true,"curve":"basis","useMaxWidth":true,"nodeSpacing":36,"rankSpacing":46},"themeVariables":{"fontFamily":"Microsoft YaHei, PingFang SC, Noto Sans CJK SC, Arial, sans-serif","fontSize":"14px","primaryColor":"#ffffff","primaryTextColor":"#111827","primaryBorderColor":"#334155","lineColor":"#475569","tertiaryColor":"#f8fafc","clusterBkg":"#ffffff","clusterBorder":"#cbd5e1"}}}%%
flowchart TD
    M0["Hardware_<br/>Init()"]
    M1["TIM1_dead<br/>_pwm_init()<br/>(999,71<br/>0,100)<br/>配置 1kHz PWM + 死区"]
    M2["motor_init()<br/>方向默认正转<br/>随后停止"]
    M3["TIM2_encode<br/>_init()<br/>编码器输入"]
    M4["StartTask<br/>内初始化 TIM4<br/>任务和信号量创建后<br/>再开测速中断"]

    STATE(("g_system<br/>State"))
    C0["MotorControl<br/>Task<br/>读取 target<br/>RPM<br/>actualRPM<br/>motorRunning"]
    C1["PID 计算<br/>得到 PWM 调整量"]
    C2["motor_pwm<br/>_set(val)<br/>上层速度接口"]
    C3["motor_dir()<br/>先全关<br/>再切方向"]
    C4["motor_speed()<br/>写 TIM1 CCR1"]
    C5["motor_start()<br/>motor_stop()<br/>PA2 使能或断电"]

    S0["TIM4 周期采样"]
    S1["get_encoder<br/>_value()<br/>读取 TIM2 计数差"]
    S2["get_speed()<br/>RPM + 中值滤波<br/>低通滤波"]
    S3["写回 actualRPM"]

    PWM["TIM1 CH1<br/>CH1N<br/>PWM 输出到 H 桥"]
    MOTOR(("电机实际转速"))
    ...
    classDef init fill:#eff6ff,stroke:#2563eb,stroke-width:1.5px,color:#0f172a;
    classDef ctrl fill:#ecfdf5,stroke:#059669,stroke-width:1.5px,color:#064e3b;
    classDef speed fill:#fff7ed,stroke:#ea580c,stroke-width:1.5px,color:#0f172a;
    classDef state fill:#fef9c3,stroke:#a16207,stroke-width:2.2px,color:#422006;
    classDef out fill:#fef2f2,stroke:#dc2626,stroke-width:1.8px,color:#7f1d1d;
    class M0,M1,M2,M3,M4 init;
    class C0,C1,C2,C3,C4,C5 ctrl;
    class S0,S1,S2,S3 speed;
    class STATE state;
    class PWM,MOTOR out;
```
...
flowchart TD
    A["A/B 相脉冲"]
    B["PA0 / PA1"]
    C["TIM2 编码器模式<br/>计数 + 判断方向"]
    D["TIM4 周期中断"]
    E["TIM4<br/>IRQHandler<br/>释放 g_speed<br/>CalcSemaphore"]
    F["SpeedCalc<br/>Task<br/>阻塞等待<br/>收到信号后运行"]
    G["get_encoder<br/>_value()<br/>TIM2_CNT<br/>+ overflow<br/>* 65536"]
    H["get_speed()<br/>计数差 -> RPM<br/>并做滤波"]
    I["actualRPM"]
    J["PID 调 PWM"]
    ...
    classDef input fill:#eff6ff,stroke:#2563eb,stroke-width:1.5px,color:#0f172a;
    classDef sample fill:#ecfdf5,stroke:#059669,stroke-width:1.5px,color:#064e3b;
    classDef calc fill:#fff7ed,stroke:#ea580c,stroke-width:1.5px,color:#0f172a;
    classDef out fill:#fef2f2,stroke:#dc2626,stroke-width:1.8px,color:#7f1d1d;
    class A,B,C input;
    class D,E,F sample;
    class G,H calc;
...
flowchart TD
    A["档位 speedLevel"]
    B["WindSpeed<br/>GetTarget<br/>RPM()<br/>低档190 / 高档220"]
    C["PID_Set<br/>Target()<br/>写入 g_speed<br/>PID.target"]
    D["TIM2 编码器<br/>TIM4 周期采样"]
    E["SpeedCalc<br/>Task"]
    F["get_speed()<br/>计数差 -> RPM<br/>滤波后得到 speed"]
    G["PID<br/>Calculate()<br/>target<br/>speed"]
    H["motor_pwm<br/>_set()<br/>方向 + PWM 控制量"]
    I["motor_speed()"]
    J["TIM_Set<br/>Compare1()<br/>TIM1, ccr<br/>修改 PWM 占空比"]
    K["电机转速变化"]
    L["编码器继续反馈"]
    ...
    classDef target fill:#eff6ff,stroke:#2563eb,stroke-width:1.5px,color:#0f172a;
    classDef feedback fill:#ecfdf5,stroke:#059669,stroke-width:1.5px,color:#064e3b;
    classDef calc fill:#fff7ed,stroke:#ea580c,stroke-width:1.5px,color:#0f172a;
    classDef output fill:#fef2f2,stroke:#dc2626,stroke-width:1.8px,color:#7f1d1d;
    class A,B,C target;
    class D,E,F,L feedback;
    class G calc;
...
### 4.6 启动任务：System_Init 和 StartTask
...
### 4.8 状态协作：g_systemState 和 g_dataMutex
...
%%{init: {"theme":"base","flowchart":{"htmlLabels":true,"curve":"basis","useMaxWidth":true,"nodeSpacing":36,"rankSpacing":46},"themeVariables":{"fontFamily":"Microsoft YaHei, PingFang SC, Noto Sans CJK SC, Arial, sans-serif","fontSize":"14px","primaryColor":"#ffffff","primaryTextColor":"#111827","primaryBorderColor":"#334155","lineColor":"#475569","tertiaryColor":"#f8fafc","clusterBkg":"#ffffff","clusterBorder":"#cbd5e1"}}}%%
flowchart TD
    DHT["DHT11<br/>温湿度"] --> SENSOR["Sensor<br/>Task<br/>500ms"]
    MQ2["MQ2 气体浓度"] --> SENSOR
    SENSOR -->|"写 temp / humi<br/>gas"| STATE(("g_system<br/>State"))

    STATE --> WIND["WindSpeed<br/>Task<br/>100ms"]
    WIND -->|"写回风速结果"| STATE

    KEY["按键事件"] --> KEYTASK["KeyScan<br/>Task<br/>10ms"]
    KEYTASK -->|"改模式 / 档位 / 启停"| STATE

    TIM4["TIM4 中断"] -->|"释放测速<br/>信号量"| SPEED["SpeedCalc<br/>Task"]
    SPEED -->|"计算 speed"| MOTOR["MotorControl<br/>Task<br/>50ms"]

    STATE --> MOTOR
    MOTOR -->|"PID / PWM<br/>状态机"| FAN["电机"]
    STATE --> UI["UIDisplay<br/>Task<br/>200ms"]
    UI --> LCD["LCD<br/>显示"]
```
...
### 4.12 移植底层：FreeRTOSConfig、SVC、SysTick、PendSV
...
%%{init: {"theme":"base","flowchart":{"htmlLabels":true,"curve":"basis","useMaxWidth":true,"nodeSpacing":36,"rankSpacing":46},"themeVariables":{"fontFamily":"Microsoft YaHei, PingFang SC, Noto Sans CJK SC, Arial, sans-serif","fontSize":"14px","primaryColor":"#ffffff","primaryTextColor":"#111827","primaryBorderColor":"#334155","lineColor":"#475569","tertiaryColor":"#f8fafc","clusterBkg":"#ffffff","clusterBorder":"#cbd5e1"}}}%%
flowchart TD
    A["PC端生成 APP.bin"] --> B["add_crc32.py<br/>计算并追加4字节CRC32"]
    B --> C["APP_crc<br/>.bin<br/>APP正文<br/>4字节CRC32"]
    C --> D["USART1 接收固件"]
    D --> E["DMA1<br/>Channel5<br/>搬运到 RAM<br/>receive_<br/>buff"]
    E --> F["DMA完成中断<br/>释放 g_iap<br/>Semaphore"]
    F --> G["iap_task 被唤醒"]
    G --> H{"CRC32校验是否通过?"}
    H -- "失败" --> I["清空<br/>receive_<br/>buff<br/>重开 DMA<br/>等待重传"]
    H -- "成功" --> J["firmwareLen<br/>= received<br/>Length - 4"]
    J --> K["检查APP入口是否合法"]
    K --> L["擦写 Flash APP区"]
    L --> M["设置APP的MSP"]
    M --> N["跳转APP<br/>Reset<br/>Handler"]
```
...
%%{init: {"theme":"base","flowchart":{"htmlLabels":true,"curve":"basis","useMaxWidth":true,"nodeSpacing":36,"rankSpacing":46},"themeVariables":{"fontFamily":"Microsoft YaHei, PingFang SC, Noto Sans CJK SC, Arial, sans-serif","fontSize":"14px","primaryColor":"#ffffff","primaryTextColor":"#111827","primaryBorderColor":"#334155","lineColor":"#475569","tertiaryColor":"#f8fafc","clusterBkg":"#ffffff","clusterBorder":"#cbd5e1"}}}%%
flowchart LR
    subgraph Flash["Flash<br/>0x08000000 起"]
        B1["Bootloader区<br/>升级逻辑"]
        A1["APP区<br/>0x0800F000<br/>FLASH_APP1<br/>ADDR"]
    end
    subgraph RAM["SRAM<br/>0x20000000 起"]
        R1["Bootloader<br/>运行 RAM<br/>栈 / 全局变量<br/>RTOS 对象"]
        R2["receive_<br/>buff<br/>0x20004000<br/>临时接收<br/>APP_crc.bin"]
    end
    R2 -->|"CRC通过后写入"| A1
```
...
**8.1 为什么在 RTOS 的多任务间通信中，使用互斥量加全局变量的形式？**
...
**8.6 请你说一下什么是 Bootloader？STM32 的启动上电流程是怎样的？**
...
**8.7 请你说一下移植 RTOS 内核时需要对哪些文件进行修改？**
...
**8.10 请你说一下什么是 PID 算法，各个参数分别解决什么问题？**
...
**8.19 请你说一下 FreeRTOS 中的动态内存管理方式**
...
**8.28 FreeRTOS 中的中断服务函数与任务间通信的安全方式，为何不建议在 ISR 中直接调用复杂函数？**
...
**8.29 RTOS 中任务优先级的分配原则，高优先级任务长期占用 CPU 会导致什么问题？**
...
此外，还可以结合非易失性存储器中的状态标记来实现断点续传。每次升级 开始前，系统会在Flash中写入特定的升级标志和进度信息。若写入新固件过程 中断电，上电后启动加载程序首先检查这些标志，发现上次升级未完成且当前活 动分区不可用或校验失败，则会重新尝试从备份区启动或从通信接口（如云端、 U盘）重新获取固件包。部分高级设计还支持从异常中恢复，通过冗余的启动代码强制进入固件更新模式，确保设备永不“变砖”，直至升级成功。
